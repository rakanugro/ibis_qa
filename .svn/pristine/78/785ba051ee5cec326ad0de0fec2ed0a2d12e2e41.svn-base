...
function getDetailContainer($in_param) {
	...
	try {
		...
		//$del_type = $xml_data->data->del_type;
		...
		//} else {
			...
			/* $getContainer="SELECT A.NO_CONTAINER,
						   ...
						   TO_CHAR (TO_DATE (A.PLUG_IN, 'YYYYMMDDHH24MISS'),
									'DD-MM-YYYY HH24:MI')
							  PLUG_IN,
							NVL((SELECT ITT_FLAG FROM ITOS_OP.CON_LISTCONT WHERE NO_CONTAINER = a.NO_CONTAINER AND ID_CLASS_CODE = a.E_I AND ID_VES_VOYAGE = b.UKKS),'N') ITT_FLAG
					  FROM    M_CYC_CONTAINER A
						   LEFT JOIN
							  M_VSB_VOYAGE B
					 ...
					 WHERE     A.VOYAGE_IN = '$vin'
						   ...
						   AND A.ACTIVE = 'Y'
						   AND UPPER (A.CONT_LOCATION) = UPPER ('Ship')
						   AND A.VESSEL_CONFIRM IS NULL
						   AND (HOLD_STATUS <> 'Y' OR hold_status IS NULL)";
		} */
		...
		while ($row_container = oci_fetch_array($query_container, OCI_ASSOC))
		{
			...
			$container_sub = array(
				...
				'comodity' => $row_container['COMODITY']
			);
