...
function getReconcilePTPKapalAutocollection($start,$end) {
  ...
  if($conn = oci_connect('KAPAL_PROD', '$Prod_k4p4lpass', '10.10.30.22:1521/peldb'))
   {
		$query2 = "select TRX_NUMBER, TO_CHAR(TRX_DATE, 'YYYYMMDDHH24MiSS') as TRX_DATE, CURRENCY_CODE, STATUS, a.NO_PPKB, KODE_KAWASAN, TAGIHAN_NOTA, c.NAMA_PERUSAHAAN  
						from ac_deduct_nota b, ptp_nota_header a left join mst_pelanggan c ON c.kd_pelanggan=a.customer_number 
						where a.trx_number=b.no_nota and TRX_DATE BETWEEN TO_DATE ('".trim($start)."', 'DD/MM/YYYY HH24:MI:SS') and TO_DATE ('".trim($end)."', 'DD/MM/YYYY HH24:MI:SS')";
		...
		if (!oci_execute($query2))
		{
			...
			<xml>
				<status>87</status>
				<message>Biller database problem;$tmp</message>
			</xml>";
...
function ePaymentBarangEdc($nota, $tgl_pelunasan, $amount, $user, $via, $nojkm, $kdbank, $company ) 
{
  ...
  if($conn = oci_connect('barang_prod', '$Prod_b4r4ngpass', '10.10.30.22:1521/peldb'))
  {
    ...
    if (!oci_execute($stid)) 
	{
		...
		$xml_str = "^87^Biller database problem^$e[message]^$tgl_pelunasan";
