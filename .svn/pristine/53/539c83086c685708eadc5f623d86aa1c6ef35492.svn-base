...
/*|
 ...
 | EDIT LOG			:   
 |*/
function getRequestReceivingHeader($in_param) {
	...
	try {
		...
		//PL/SQL
		$query = "select REQUEST_ID, TO_CHAR(REQUEST_DATE,'dd-mm-yyyy hh24:mi') REQUEST_DATE, PORT_ID, TERMINAL_ID, STATUS_REQ, ADDITIONAL_FIELD3, ADDITIONAL_FIELD2,ADDITIONAL_DATE, VESSEL ||' '||VOYAGE_IN||'-'||VOYAGE_OUT AS VESVOY, BILLER_REQUEST_ID 
						from transaction_log where CUSTOMER_ID='$agent_id' and request_id like 'A%' order by REQUEST_ID desc";
		...
		while($row = oci_fetch_array($query_request, OCI_ASSOC))
		{
			...
			//return $qx;
			//QUERY
			if(!checkOriSQL($conn['billing'][0],$qx,$qx_out,$err,$debug)) goto Err; 
