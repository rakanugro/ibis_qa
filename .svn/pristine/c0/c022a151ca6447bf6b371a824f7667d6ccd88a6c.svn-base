...
function getHTMLCardContainer($in_param) {
	...
	try {
		...
		if($port_code == 'IDJKT' && $terminal_code == 'ITOST'){
			if(!checkOriSQL($conn['ori']['billing_idjkt_itost'],$query_card,$query_card_,$err,$debug)) goto Err;
		}
		...
		if($port_code == 'IDJKT' && $terminal_code == 'ITOST'){
			if(!checkOriSQL($conn['ori']['billing_idjkt_itost'],$query_vessel,$query_vessel_,$err,$debug)) goto Err;
		}
