...
function AddContainerBM($in_param) {
	...
	try {
		...
		//return $query_getheader;	
			...
			if(!checkOriSQL($conn['ori']['container'],$sql_vsb,$get_vsb,$err,$debug,$bind_param)) goto Err;
			...
			try {
				...
				if (($terminal_code  = 'T3I') OR ($terminal_code  = '009D')){
					if ($tipebm == 'CALDG'){
						if(!checkOriSQL($conn['ori']['billing'],$sql2,$query_,$err,$debug,$param_b_var2)) goto Err;
						$data = $param_b_var2['v_msg'];	
					} else {
						if(!checkOriSQL($conn['ori']['billing'],$sqlt3i,$query_,$err,$debug,$param_b_vart3i)) goto Err;
					...
					if ($tipebm == 'CALDG'){
						...
						if(!checkOriSQL($conn['ori']['billing'],$sql,$query_,$err,$debug,$param_b_var)) goto Err;
