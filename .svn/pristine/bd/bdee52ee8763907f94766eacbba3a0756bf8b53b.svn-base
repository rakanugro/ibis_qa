...
function ePaymentInquiry($notanumber, $userid) {
  ...
  if($kodeperusahaan=="9")//petikemas
  {
	...
	if($kodeperusahaan=="015")//pnk
		$conn = oci_connect('BILLING', 'billingpnkkitasemua', 'ipc-domdb-scan.indonesiaport.co.id/PNKDB');
	else if($kodeperusahaan=="011")//jkt
		$conn = oci_connect('BILLING', 'billing', '192.168.29.85/MTDB');
	else if($kodeperusahaan=="020")//panjang
		$conn = oci_connect('BILLING', 'billing', '10.10.33.161/PJGDB');
	else if($kodeperusahaan=="040")//palembang
		$conn = oci_connect('BILLING', 'billing', '10.10.33.161/PLGDB');
	else 
		return "^021^KODE PERUSAHAAN BELUM TERDAFTAR^^^^^^^^^^^^^^^^^^^^";
  }
  else if($kodeperusahaan=="0"||$kodeperusahaan=="1")//uster
  {
	$conn = oci_connect('USTER', 'uster', 'ipc-domdb-scan.indonesiaport.co.id/PNKDB');//pnk
  }
  else 
...
function ePaymentPaid($nomor_nota, $user_id, $bank_id, $paid_date, $paid_channel) {
  ...
  {
	$conn = oci_connect('USTER', 'uster', 'ipc-domdb-scan.indonesiaport.co.id/PNKDB');
  }
  else 
	return "^022^KODE PERUSAHAAN BELUM TERDAFTAR^^^^^^^^^^^^^^^^^^^^";
...
function getReconcilePtkmPnk($start,$end,$paid_channel,$userid="",$bankid="",$orgid="") {
   ...
   // db connection
  if($conn = oci_connect('BILLING', 'billingpnkkitasemua', 'ipc-domdb-scan.indonesiaport.co.id/PNKDB'))
   {
...
function getReconcilePtkmPjg($start,$end,$paid_channel,$userid="",$bankid="",$orgid="") {
   ...
   // db connection
  if($conn = oci_connect('BILLING', 'billing', '10.10.33.161/PJGDB'))
  {
...
function getReconcilePtkmPlg($start,$end,$paid_channel,$userid="",$bankid="",$orgid="") {
   ...
   // db connection
  if($conn = oci_connect('BILLING', 'billing', '10.10.33.161/PLGDB'))
  {
...
function getReconcilePtkm009($start,$end,$paid_channel,$userid="",$bankid="",$orgid="") {
   ...
   // db connection
  if($conn = oci_connect('BILLING', 'billing', '192.168.29.85/MTDB'))
   {
...
function getReconcileUsterPnk($start,$end,$paid_channel,$userid="",$bankid="",$orgid="") {
   ...
   // db connection
  if($conn = oci_connect('USTER', 'uster', 'ipc-domdb-scan.indonesiaport.co.id/PNKDB'))
   {
		...
		$query2 = "select trx_number as no_nota, NO_REQUEST as no_request, '' as cust_name,  total, ppn, kredit, 
        TO_CHAR(tgl_pelunasan, 'YYYYMMDDHH24MiSS') as DATE_PAID, userid_lunas as user_paid, receipt_method as paid_via, receipt_account, 
        b.nm_pbm 
        from itpk_nota_header a left join mst_pelanggan b on a.customer_number = b.no_account_pbm  
