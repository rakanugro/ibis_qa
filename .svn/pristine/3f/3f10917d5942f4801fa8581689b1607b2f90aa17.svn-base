...
/* 			$( "#bl_numb" ).autocomplete({
	...
	$( "#cargo_name" ).autocomplete({
			minLength: 5,
			source: function(request, response) {
				$.getJSON("<?=ROOT?>autocomplete/getHsDelivery?",{  term: $( "#cargo_name" ).val(),ukk: $('#ukk').val(),bl_numb: $('#bl_numb').val(),
																			mv_type: $('#mv_type').val(),
																			  port: $('#port').val()
																			 }, response);
				},
			...
			{
				$( "#cargo_name" ).val( ui.item.CARGO_NAME);
				$( "#cargo_id" ).val( ui.item.ID_CARGO_OM);
				$( "#hs_code" ).val( ui.item.HS_CODE);
				$( "#ton" ).val( ui.item.WEIGHT);
				$( "#tonval" ).val( ui.item.WEIGHT);
				$( "#cubic" ).val( ui.item.VOLUME);
				$( "#cubicval" ).val( ui.item.VOLUME);
				$( "#qty" ).val( ui.item.QUANTITY);
				$( "#qtyval" ).val( ui.item.QUANTITY);
				return false;
			}
		}).data( "uiAutocomplete" )._renderItem = function( ul, item )
		{
...
else 	if(cubic=='' && ton=='' && qty=='' )
{
	...
	if ((Number(cubic)) > (Number(cubicval)))
				...
				<table class="table table-striped table-hover" id='list_cargo_table' style='font-size:12px;'>
					...
					<tbody>
						<tr>
							...
							<td><select id="hz" name="hz" class="form-control" style="width:70px;">
									<option value="N">N</option>
									<option value="Y">Y</option>
								</select></td>
							<td><select id="ds" name="ds" class="form-control" style="width:70px;">
									<option value="N">N</option>
									<option value="Y">Y</option>
								</select></td>
							<td>
