...
function createRequestDeliveryPerp($in_param) {
	...
	try {
		...
		$conn['container'][0] = $conn['ori']['billing'];
        ...
        if($request_no=="")
		{
            ...
            if(!checkOriSQL($conn['ori']['ibis'],$query_numbering,$query_numbering_,$err,$bind_param)) goto Err;
            ...
            //QUERY
            if(!checkOriSQL($conn['container'][0],$createRequest,$query_request,$err)) goto Err;
            
            // get Biller Request Id
            $getReqNum = "SELECT MAX(ID_REQ) ID_REQ FROM REQ_DELIVERY_H WHERE ID_REQ LIKE 'SP2%'";
        
            if(!checkOriSQL($conn['container'][0],$getReqNum,$queryReqNum,$err)) goto Err;
            ...
            if(!checkOriSQL($conn['container'][0],$query,$query_,$err,$bind_param)) goto Err;
			...
			$createDetails = "INSERT INTO req_delivery_d (ID_REQ,
                                        NO_CONTAINER,
                                        SIZE_CONT,
                                        TYPE_CONT,
                                        STATUS_CONT,
			...
			//return $createDetails;
            if(!checkOriSQL($conn['container'][0],$createDetails,$query_details,$err)) goto Err;
            ...
            if(!checkOriSQL($conn['container'][0],$getDataRequest,$queryDataRequest,$err)) goto Err;
            //return $getDataRequest;   
            //FETCH QUERY
            if ($rowDataRequest = oci_fetch_array($queryDataRequest, OCI_ASSOC)){
				//return $rowDataRequest["ICUST_DSTN"];
                ...
                $query = "insert into transaction_log (
                    ...
                    VESSEL, VOYAGE_IN, VOYAGE_OUT, ADDITIONAL_DATE,LAST_USER_ACTIVITY_USERID 
                    )
                values
                (TRIM('$request_no'), TRIM('$biller_request_no'), 'PTKM07','PERPANJANGAN DELIVERY'
                    ...
                    ,'$no_sppb', '', '$no_do','' ,'',SYSDATE, '$id_user_eservice', '' , '', '' ,'','', '', '', ''
                    ,'$ukk','$vessel','$voyage_in','$voyage_out', TO_DATE('$tgl_perp','DD-MM-YYYY'), '$id_user_eservice'
                )";
                ...
                if(!checkOriSQL($conn['container'][0],$query_proc_d,$query_proc_d_,$err,$bind_param_proc_d)) goto Err; */
            }
        }

		//return $biller_request_no;
		$out_data = array();
