...
function getPDFNotaContainer($in_param) {
	...
	try {
		...
		if($port_code == 'IDJKT' && $terminal_code == 'ITOST'){
			if(!checkOriSQL($conn['ori']['billing_idjkt_itost'],$query_id_terminal,$queryIdTerminal,$err,$debug)) goto Err;
		}
		...
		if($port_code == 'IDJKT' && $terminal_code == 'ITOST'){
			if(!checkOriSQL($conn['ori']['billing_idjkt_itost'],$query,$query_header,$err,$debug)) goto Err;
		}
		...
		if($port_code == 'IDJKT' && $terminal_code == 'ITOST'){
			if(!checkOriSQL($conn['ori']['billing_idjkt_itost'],$query_dtl,$queryDetail,$err,$debug)) goto Err;
		}
