...
function getPDFDTJK($in_param) {
	...
	try {
		...
		$getHeader="SELECT
				  ...
				  to_char(sysdate,'DD-MM-YYYY') TGL_DTJK, all_general_pkg.get_subsidiary_branch_name('KAPAL','01',TO_DATE(TO_CHAR(PKK.TGL_JAM_TIBA,'dd-mm-yyyy'),'dd-mm-yyyy')) CABANG1
				FROM
				  PPKB,
				  PKK,
				  MST_KAPAL_AGEN,
		...
		if ($row = oci_fetch_array($query_header, OCI_ASSOC))
		{
			...
			<tr>
			  ...
			  <td width="500">&nbsp;</td>
			  <td colspan="2"><div style="font-size:'.$font_detail.'" align="left">FM.01/01/01/66</div></td>
			</tr>
			<tr>
			  <td width="270"><div style="font-size:'.$font_detail.'" align="left"></div></td>
			  <td width="500">&nbsp;</td>
