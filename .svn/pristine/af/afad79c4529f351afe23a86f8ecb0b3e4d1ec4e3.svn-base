...
/*|
 ...
 | EDIT LOG			: 
 |*/
function insertDeliveryOrder($in_param) {
	...
	try {
		...
		$terminal_id = $xml_data->terminal_id;
		...
		for($i=0;$i<count($d_o);$i++)
		{
			...
			{									
				...
				$sql = "select count(do_number) as hasil from MST_DOONLINE_FRMILCS where DO_NUMBER='$do_no' AND CONTAINER_NUMBER='$no'";
				if(!checkOriSQL($conn['ori']['ibis'],$sql,$query_result,$err)) goto Err;
				
				$row = oci_fetch_array($query_result, OCI_ASSOC);
				if($row["HASIL"]>0)
				{
					$sql = "update MST_DOONLINE_FRMILCS set VES_CALLSIGN='$ves_callsign',
															VES_VOYIN='$ves_voyin',
				...
				{
					$sql = "insert into MST_DOONLINE_FRMILCS (
															DO_NUMBER,
															VES_CALLSIGN,
															VES_VOYIN,
															VES_VOYOUT,
				...
				//QUERY
				if(!checkOriSQL($conn['ori']['ibis'],$sql,$query_result,$err)) goto Err;
				//if(!checkOriSQL($conn['ori']['billing_idjkt_t3i'],$sql,$query_result,$err)) goto Err;
