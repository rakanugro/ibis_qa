...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ Terminal 3 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//	
   if ($conn = oci_connect('ITOS_OP', 'itos_OP', '10.10.33.25/to3')) { 		
...
function getITT_Booking($service) {
    ...
    //reset stagging lini 2
    if ($conn6 = oci_connect('LINEOS', 'ligneOS', '10.10.33.25/orcl')) {
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ Terminal 3 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//   
   if ($conn = oci_connect('ITOS_OP', 'itos_OP', '10.10.33.25/to3')) { 		
        ...
        while ($row = oci_fetch_array($query2, OCI_ASSOC))
        {
			...
			//==== Insert Stagging Lini 2 (Header) ===//
			if ($conn2 = oci_connect('LINEOS', 'ligneOS', '10.10.33.25/orcl')) 
			{
			   $idplp = "";
			   $atd = "";
			   $callsign = "";
			...
			while ($rowd = oci_fetch_array($queryDtl, OCI_ASSOC))
			{
				...
				if ($conn3 = oci_connect('LINEOS', 'ligneOS', '10.10.33.25/orcl'))
				{
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ Terminal 2 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//   
   if ($conn = oci_connect('ITOS_OP', 'itos_OP', '10.10.33.25/to2')) { 		
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ Terminal 1 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//   
   if ($conn = oci_connect('ITOS_OP', 'itos_OP', '10.10.33.25/to1')) { 		
...
function setVerify_ITT($idrec) {
   ...
   if ($conn = oci_connect('LINEOS', 'ligneOS', '10.10.33.25/orcl')) { 		
        ...
        while ($row = oci_fetch_array($query2, OCI_ASSOC))
        {
			...
			$nocont = $row['NO_CONT'];
			$point = $row['ITT_POINT'];
			$kdtpl1 = $row['ORIGIN_TERM'];
			
			if($kdtpl1=='TER3')
			{
				// db connection
				if ($conn2 = oci_connect('ITOS_OP', 'itos_OP', '10.10.33.25/to3')) {
			...
			{
				// db connection
				if ($conn2 = oci_connect('ITOS_OP', 'itos_OP', '10.10.33.25/to2')) {
			...
			{
				// db connection
				if ($conn2 = oci_connect('ITOS_OP', 'itos_OP', '10.10.33.25/to1')) {
