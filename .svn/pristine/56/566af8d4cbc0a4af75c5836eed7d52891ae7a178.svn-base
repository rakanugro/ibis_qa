...
function getCustomerDetail($in_param) {
	...
	try {
		...
		$userinfo="";
		//PL/SQL
		$getUserinfo = "select email,handphone,substr(password,-5) pdf_password from mst_user where username='$user_id'";
		//QUERY
		if(!checkOriSQL($conn['ori']['ibis'],$getUserinfo,$query_user,$err)) goto Err;
		//FETCH QUERY
		$row_user = oci_fetch_array($query_user, OCI_ASSOC);

		$userinfo = array (
					'uphone' => $row_user[HANDPHONE],
					'uemail' => $row_user[EMAIL],
					'pdf_password' => $row_user[PDF_PASSWORD],
		);
		...
		if(!$find)
		{//biar gak error klw data gak ditemukan
			$service_list_sub = array (
											'service' => ''
										);
