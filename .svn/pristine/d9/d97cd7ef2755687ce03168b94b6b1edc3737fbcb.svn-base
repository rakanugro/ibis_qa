			...
			############################# TEMAS ###################################################
				...
				while ($row_vessel_ctp = oci_fetch_array($getvvdctp, OCI_ASSOC))
				{
					if ($row_vessel_ctp[TERMINAL] == '009'){
							...
							$qqty = "select * from (
									select count(no_container) qty_20 from simpb.coarri_ptp where vessel = '".$row_vessel_ctp[VESSEL]."' and voyage_in = '".$row_vessel_ctp[VOYAGE_IN]."'
									and e_i = 'E' and substr(iso_code,0,1) =2),
									(select count(no_container) qty_40 from simpb.coarri_ptp where vessel = '".$row_vessel_ctp[VESSEL]."' and voyage_in = '".$row_vessel_ctp[VOYAGE_IN]."'
									and e_i = 'E' and substr(iso_code,0,1)= 4),
									(select count(no_container) qty_45 from simpb.coarri_ptp where vessel = '".$row_vessel_ctp[VESSEL]."' and voyage_in = '".$row_vessel_ctp[VOYAGE_IN]."'
									and e_i = 'E' and substr(iso_code,0,1) not in (2,4))";
