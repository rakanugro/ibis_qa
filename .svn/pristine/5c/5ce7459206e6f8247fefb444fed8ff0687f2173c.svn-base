...
//        die;
        ...
        {
            $edit_view_link = '<a  class=\'btn btn-primary\'  href=\'' . ROOT . 'customer/index/{TICKET_NUMBER}\'><i class=\'fa fa-pencil\'></i></a>';
            if ($this->session->userdata('group_phd') != 'p' && $this->session->userdata('group_phd') != '8' && $this->session->userdata('group_phd') != 'm')//user
            {
                $this->table->set_heading('No', 'Nomor Tiket', 'Judul', 'Tipe', 'Waktu Pembuatan', 'Status', 'Lihat', 'Respon Terakhir');
                $max_row = "200";
        ...
        //form section
        {
            if ($ticket_number == "")//new ticket form
            {
                ...
                $data['action'] = ROOT . 'customer/create_ticket/';
            ...
            } else//view edit form
            {
                ...
                $data['action'] = ROOT . 'customer/create_ticket_message/' . $ticket_number;
                $data['submit_button'] = '';
            }
        ...
        $this->common_loader($data, 'pages/customer/ticket');
    }
    ...
    public function search_ticket()
    {
        ...
        $this->load->view('pages/customer/search_ticket', $data);
    }
    ...
    public function create_ticket()
    {
        ...
        if (!$this->session->userdata('uname_phd')) {
            ...
            if (($_POST['title'] === null) and ($_POST['message'] === null) and ($_POST['request_number'] === null)) {
                ...
                // End Added by EDII
                redirect(ROOT . 'customer', 'refresh');
            }
    ...
    public function create_ticket_message($ticket_number)
    {
        if (!$this->session->userdata('uname_phd')) {
            ...
            if (($ticket_number === null) and (htmLawed($_POST['message2']) === null)) {
                ...
                // End Added by EDII
                redirect($base_url . 'customer/index/' . $ticket_number, 'refresh');
            }
        }
    }
    ...
    public function contact_us()
    {
        ...
        $this->breadcrumbs->push('Kontak', '/customer/contact_us');
        $this->breadcrumbs->unshift('Home', '/');
        $data['breadcrumbs'] = $this->breadcrumbs->show();
        ...
        $this->common_loader($data, 'pages/customer/contact_us');
    }
    ...
    public function my_profile()
    {
        ...
        $this->common_loader($data, 'pages/customer/my_profile');
    }
