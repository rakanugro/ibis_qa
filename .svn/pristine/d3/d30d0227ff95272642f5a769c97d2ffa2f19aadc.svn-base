 ...
 class GetHsCodeNpk extends CI_controller {
		...
		function index() 
		{
			...
			$in_data="<root>
				...
				<data>
					...
					<cargo_owner>".$cargo_owner."</cargo_owner>
					<port_code>".$port[0]."</port_code>
					<terminal_code>".$port[1]."</terminal_code>
				</data>
			</root>";

			if(!$this->nusoap_lib->call_wsdl(ORDER_MGT,"getHsCodeNpk",array("in_data" => "$in_data"),$result))
			{
				echo $result;
				die;
			}
				  ...
				  for($i=0;$i<count($obj->data->hscode);$i++)
					{
						...
						$temp['WEIGHT']=$obj->data->hscode[$i]->weight;
						$temp['VOLUME']=$obj->data->hscode[$i]->volume;
						$temp['QUANTITY']=$obj->data->hscode[$i]->quantity;
