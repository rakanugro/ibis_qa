...
class Nusoap_lib
{
	  ...
	  function call_wsdl($wsdl,$modul,$in_data,&$result)
	  {
		...
		//$client->setCredentials("npk_billing","npk_billing","basic");
        $result  = $client->call($modul, $in_data);
      //  $resultx = $client->setCredentials($username, $password, $authtype = 'basic', $certRequest = array());
...
/*	  function setCredentials($wsdl, $username, $password, $authtype = 'basic', $certRequest = array()) {
		$client 	= new nusoap_client($wsdl);
		$client->setCredentials($username, $password, $authtype, $certRequest);	  
	  }*/
