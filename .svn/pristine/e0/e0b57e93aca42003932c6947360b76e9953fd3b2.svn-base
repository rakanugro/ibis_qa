...
function saveDetailReqPerp($in_param) {
	...
	try {
		...
		//QUERY
		if(!checkOriSQL($conn['ori']['ibis'],$createRequest,$query_request,$err,$debug)) goto Err;
		
		$getReqNum = "SELECT COUNT(DISTINCT NO_CONTAINER) JUM FROM REQ_DELIVERY_D WHERE ID_REQ LIKE '$id_request'";
	
		if(!checkOriSQL($conn['ori']['ibis'],$getReqNum,$queryReqNum,$err,$debug)) goto Err;
		...
		if(!checkOriSQL($conn['ori']['ibis'],$updQty,$queryUpdQty,$err,$debug)) goto Err;	
		
		$q_getreq = "select * from req_delivery_h where id_req = '$old_request'";
		
		if(!checkOriSQL($conn['ori']['ibis'],$q_getreq,$querygetreq,$err,$debug)) goto Err;	
		...
		if($port_code=="IDJKT"&&$terminal_code=="ITOST"){
			...
			if(!checkOriSQL($conn['ori']['billing_idjkt_itost'],$query,$query_,$err,$debug,$param_b_var)) goto Err;	
