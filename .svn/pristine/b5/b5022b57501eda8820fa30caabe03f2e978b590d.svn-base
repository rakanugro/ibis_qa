...
function populateInvoice($in_no_nota) {
	...
	try {
		...
		if(!checkOriSQL($conn['ori']['orafin'],$sql,$query_tth_nota_all2,$err,$debug)) goto Err;

		//get nota header from mti
		if(!checkOriSQL($conn['ori']['mti'],getCountNotaHeader($in_no_nota),$query_nota_header,$err,$debug)) goto Err;
		...
		//get nota header from mti
		if(!checkOriSQL($conn['ori']['mti'],getNotaHeader($in_no_nota),$query_nota_header,$err,$debug)) goto Err;
		...
		while ($row = oci_fetch_array($query_nota_header, OCI_ASSOC))
		{
			...
			if(!checkOriSQL($conn['ori']['orafin'],$sql_get_ptp_cust_no,$query_mti_customer,$err,$debug)) goto Err;
			...
			if($row_mti['JUMLAH']==0)
			{
				...
				if(!checkOriSQL($conn['ori']['mti'],$sql,$query_tbl_tth_nota_all2,$err,$debug)) goto Err;
			...
			if(!checkOriSQLAutoCommit($conn['ori']['orafin'],$sql,$query_tth_nota_all2,$err,$debug)) goto Err;
			...
			if(!checkOriSQL($conn['ori']['mti'],getCountNotaLines($in_no_nota),$query_ttr_nota_all,$err,$debug)) goto Err;
			...
			if(!checkOriSQL($conn['ori']['mti'],$sql,$query_sum_total_tarif,$err,$debug)) goto Err;
			...
			/*get lines from mti*/
			if(!checkOriSQL($conn['ori']['mti'],getNotaLines($in_no_nota),$query_ttr_nota_all,$err,$debug)) goto Err;
			...
			{  
				...
				if(!checkOriSQLAutoCommit($conn['ori']['orafin'],$sql,$query_ttr_nota_all2,$err,$debug)) goto Err;
...
function getCustomerOrafin($customer_name, $customer_number, $insert_date_start, $insert_date_end) {
	...
	try {
		...
		if(!checkOriSQL($conn['ori']['orafin'],$sql_get_cust,$query_get_customer,$err,$debug)) goto Err;
