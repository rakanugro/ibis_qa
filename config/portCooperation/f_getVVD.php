...
function getVVD($in_param) {
	...
	try {
		...
		//PL/SQL
		//if($polpod == 'JKTSBY'){
			$getVessel = " SELECT * FROM (SELECT vessel,
						 voyage_in,
						 voyage_out,
						 call_sign,
						 operator_id,
				   ...
				   WHERE     a.voyage = b.vsb_voyp_voyage
						 ...
						 vsb_voyp_port,
						 'T2' terminal
					FROM itos_repo.m_vsb_voyage@itost2link a, itos_repo.m_vsb_voyage_port@itost2link b
				   WHERE     a.voyage = b.vsb_voyp_voyage
						 AND a.vessel_code = b.vsb_voyp_vessel
						 AND vsb_voyp_port = 'IDSUB'
						 UNION
						...
						to_char(to_date(eta,'rrrrmmddhh24miss'),'dd/mm/rrrr hh24:mi:ss') eta,
						 ...
						 vsb_voyp_port,
						 'T3' terminal
					FROM itos_repo.m_vsb_voyage@itost3link a, itos_repo.m_vsb_voyage_port@itost3link b
				   WHERE     a.voyage = b.vsb_voyp_voyage
						 AND a.vessel_code = b.vsb_voyp_vessel
						 AND vsb_voyp_port = 'IDSUB') where 
						trunc(to_date(eta,'dd/mm/rrrr hh24:mi:ss')) between to_date('$startDate','dd-mm-rrrr') and to_date('$endDate','dd-mm-rrrr')";
			
			//$query_vessel = "";
		
			if(!checkOriSQL($conn['container'],$getVessel,$query_vessel,$err,$debug)) goto Err;
		/* }
		else if($polpod =='SBYJKT'){ */
