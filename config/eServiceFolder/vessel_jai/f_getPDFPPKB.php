...
function getPDFPPKB($in_param) {
	...
	try {
		...
		$getHeader="select mst_jenis_kapal.nama_jenis_kapal, mst_kapal.nm_kapal,
			ppkb_detail.labuh,
			ppkb_detail.pandu,
			ppkb_detail.pandu_laut,
			ppkb_detail.tunda,
			ppkb_detail.tunda_laut,
			ppkb_detail.kepil,
			ppkb_detail.tambat,
			ppkb_detail.air,
			ppkb_pandu.validasi,
		   ...
		   ppkb.embarkasi, ppkb.debarkasi,
		   --to_char(ppkb_labuh.tgl_jam_pmt_mlabuh, 'dd-mm-yyyy hh24:mi')tgl_jam_pmt_mlabuh,
		   ...
		   to_char(ppkb_pandu_laut.tgl_jam_pmt_pandu_d, 'dd-mm-yyyy hh24:mi')tgl_jam_pmt_pandu_laut_d, 
		   ...
		   to_char(ppkb_tunda_laut.tgl_jam_pmt_tunda_d, 'dd-mm-yyyy hh24:mi')tgl_jam_pmt_tunda_laut_d,
		   ...
		   (select cargo_name from BARANG_CABANG.tr_cargotype where cargotype_id = rpk.jns_muatan_bongkar) jns_muatan_bongkar,
		   (select cargo_name from BARANG_CABANG.tr_cargotype where cargotype_id = rpk.jns_muatan_muat) jns_muatan_muat, 
		   ...
		   to_char(ppkb.TGL_JAM_BERANGKAT_PPKB,'dd-mm-yyyy hh24:mi') TGL_JAM_BERANGKAT_PPKB, ppkb_detail.is_emergency, ppkb_detail.KD_PPKB_STATUS,decode(rpk.ex_kapal,'0','',(select NM_KAPAL from MST_KAPAL where MST_KAPAL.KD_KAPAL = rpk.ex_kapal)) ex_kapal,
		   to_char(trunc(tgl_jam_tiba),'rrrrmmdd') tgl,
		   pkk.KD_KADE
	  from ppkb,
		   pkk,
		   mst_kapal_agen,
		   ...
		   mst_service_code,
		   --v_uper,
		   BARANG_CABANG.ttm_oprplan ttm_oprplan
	 where      ppkb_detail.kd_rpk = rpk.kd_rpk (+)
            ...
            and mst_kapal_agen.kd_cabang=F_KD_CABANG_BY_UKK(pkk.no_ukk)
		...
		############
		# Get Currency per 1 Juli 2015
					
		if($row['TGL'] >= 20150701 && $row['KD_PELAYARAN'] == '1'){		
			$sql_curr = "select SIGN_CURRENCY from v_uper where kd_ppkb = '".$no_ppkb."' and ppkb_ke = '".$ppkb_ke."' and cc = 'b'";
			if(!checkOriSQL($conn['ori']['kapal'],$sql_curr,$query_curr,$err)) goto Err;
			$row_curr = oci_fetch_array($query_curr, OCI_ASSOC);
			$row["SIGN_CURRENCY"] = $row_curr["SIGN_CURRENCY"];
			
			############
			# Get Kurs Detail per 1 Juli 2015
			$sql_kurs = "SELECT TO_CHAR(KURS_DATE,'DD MON RRRR') KURS_DATE,  NUM2CHARFORMATDUIT_RP(KURS_RATE) KURS_RATE FROM PPKB_DETAIL WHERE KD_PPKB = '".$no_ppkb."' AND PPKB_KE= ".$ppkb_ke."";
			if(!checkOriSQL($conn['ori']['kapal'],$sql_kurs,$query_kurs,$err)) goto Err;
			$row_kurs = oci_fetch_array($query_kurs, OCI_ASSOC);
			
			$kurs_rate = "Kurs yang berlaku:   ".$row['SIGN_CURRENCY']." ".$row_kurs['KURS_RATE'];
		}
		...
		if(!checkOriSQL($conn['ori']['kapal'],$uper_cms,$query_cek,$err)) goto Err;
		$cek_cms = oci_fetch_array($query_cek, OCI_ASSOC);
		
		if ($cek_cms["KD_AGEN"]!="") {
			$row["NAMA_THREE_PARTIED"] = "CMS";
			$row["JUMLAH"] = "0";		
			$row["KD_THREE_PARTIED"] = "1";
		} else {
			$sql_uper = "(select num2charformatduit_rp(AMOUNT) as JUMLAH from v_data_uper		
											where kd_ppkb = '".$no_ppkb."'		
											and ppkb_ke = '".$ppkb_ke."'
											)";
			if(!checkOriSQL($conn['ori']['kapal'],$sql_uper,$query_uper,$err)) goto Err;
			$cek_uper = oci_fetch_array($query_uper, OCI_ASSOC);										
			$row["NAMA_THREE_PARTIED"] = "UPER";
			$row["JUMLAH"] = $cek_uper["JUMLAH"];		
			$row["KD_THREE_PARTIED"] = "2";
		}
		
		// $row['JENIS_PERMINTAAN'] = ($row['LABUH'] == 1)?'LABUH ':'';
		$row['JENIS_PERMINTAAN'] .= ($row['PANDU'] == 1)?'PANDU ':'';
		$row['JENIS_PERMINTAAN'] .= ($row['PANDU_LAUT'] == 1)?'PANDU_LAUT ':'';
		$row['JENIS_PERMINTAAN'] .= ($row['TUNDA'] == 1)?'TUNDA ':'';
		$row['JENIS_PERMINTAAN'] .= ($row['TUNDA_LAUT'] == 1)?'TUNDA_LAUT ':'';
        ...
        from BARANG_CABANG.TTM_OPRPLAN where opr_plan_id = '".$opr_id."'";
		...
		if( ($row_cms["STAT_CMS_DOLLAR"] == 1) & ($row_cms["STAT_CMS_RUPIAH"] == 0) & ($row["KD_PELAYARAN"] == 2)  ){
			...
			$sql_wajib_uper = "select SIGN_CURRENCY, JUMLAH from v_uper
									...
									and ppkb_ke = '".$ppkb_ke."'
									and cc = 'b'";
			if(!checkOriSQL($conn['ori']['kapal'],$sql_wajib_uper,$query_wajib_uper,$err)) goto Err;
...
$isi = '
	<div style="font-family:Serif; font-size:8pt;">
      ...
      <div style="text-align:center;font-size:12pt;font-weight:bold;"> PERMINTAAN PELAYANAN PEMANDUAN DAN PENUNDAAN <br />
        (&nbsp;P&nbsp;P&nbsp;P&nbsp;P&nbsp;) </div>
      <br />
      <table  border="0" cellpadding="0" cellspacing="1">
        <tr>
          ...
          <td width="10">:</td>
          <td width="200" style="font-size:10pt"><strong>'.$row[SERVICE_CODE].' '.$info_tarif.'</strong></td>
      ...
      <table border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          <td width="60">Pemanduan Laut</td>
          <td width="10">:</td>
          <td width="150" style="font-size:10pt">'.$row[TGL_JAM_PMT_PANDU_LAUT].'</td>
          <td width="30">&nbsp;</td>
          <td width="200" style="font-size:10pt">'.$row[TGL_JAM_PMT_PANDU_LAUT_D].'</td>
          <td width="150" align="left"><strong>'.$row[VALIDASI].'</strong></td>
        </tr>
        <tr>
        ...
        <tr>
          <td width="60" >Penundaan Laut</td>
          <td width="10" >:</td>
          <td width="150" style="font-size:10pt">'.$row[TGL_JAM_PMT_TUNDA_LAUT].'</td>
          <td width="30" >&nbsp;</td>
          <td width="200" style="font-size:10pt">'.$row[TGL_JAM_PMT_TUNDA_LAUT_D].'</td>
          <td width="150" align="center">&nbsp;</td>
        </tr>
        <!--tr>
      ...
      <div style="font-size:7pt" align="center">
        <table border="B dashed" cellspacing="8" cellpadding="">
          <tr>
            <td width="160" align="center"><CABANG PELABUHAN '.$row[CABANG].'<br />
              GENERAL MANAGER <br/>
              <br/>
              <br/></td>
