	...
	try {
		...
		*/
		...
		if($info['bentuk_3a']!="") {
			...
			//QUERY
			if(!checkOriSQL($connection,$getNotaInfo,$query_nota,$err)) goto Err;
			//FETCH QUERY
		...
		//get billing info
		//PL/SQL
		$getBillingInfo = "SELECT A.BENTUK_3A NO_NOTA, B.SIGN_CURRENCY, (REPLACE(B.PPN_DIKENAKAN_10,'.','')) PPN, B.JML_TAGIHAN_KURS JUMLAH_TAGIHAN, D.EMAIL_PERUSAHAAN, D.ALAMAT_PERUSAHAAN, (B.UANG_JAMINAN) UANG_JAMINAN, (REPLACE(B.SISA_UPER,'.','')) SISA_UPER, (REPLACE(B.PIUTANG,'.','')) PIUTANG/*, NVL((SELECT SUM(NOMINAL_UPER) FROM AC_PPKB_LOG WHERE NO_PKK=B.NO_UKK),0) JUMLAH_HOLD_AC, NVL((SELECT SUM(TAGIHAN_NOTA) FROM AC_DEDUCT_NOTA WHERE NO_PKK=B.NO_UKK),0) DEDUCT_AC*/
            ...
            WHERE A.NO_UKK=B.NO_UKK
                ...
                AND B.NO_UKK='$pkk'";
		//QUERY
		if(!checkOriSQL($conn['ori']['kapal'],$getBillingInfo,$query_billing,$err)) goto Err;
		//FETCH QUERY
		while ($row_billing = oci_fetch_array($query_billing, OCI_ASSOC))
		{
			...
			$billing = array(
							...
							'kd_currency' => /*$row_billing[SIGN_CURRENCY]*/ "Rp.",
							'ppn' => (int)$row_billing[PPN],
							// 'jumlah_tagihan' => (int)$row_billing[JUMLAH_TAGIHAN],
							'jumlah_tagihan2' => $row_billing[JUMLAH_TAGIHAN],
							...
							'uang_jaminan2' => number_format((int)$row_billing[UANG_JAMINAN],2,',','.'),
							'sisa_uper' => (int)$row_billing[SISA_UPER],
							'sisa_uper2' => number_format((int)$row_billing[SISA_UPER],2,',','.'),
							'piutang' => (int)$row_billing[PIUTANG],
							'piutang2' => number_format((int)$row_billing[PIUTANG],2,',','.')/*,
		...
		//QUERY
		if(!checkOriSQL($conn['ori']['kapal'],$getRPKInfo,$query_rpk,$err)) goto Err;
		//FETCH QUERY
		...
		where v_list_pkk_ibis.no_ukk=ppkb.no_ukk AND ppkb.kd_ppkb=ppkb_detail.kd_ppkb AND ppkb_detail.kd_ppkb_status=status_ppkb.kd_ppkb_status and ppkb_detail.kd_service_code=mst_service_code.kd_service_code and ppkb.no_ukk='$pkk'
		order by ppkb_detail.ppkb_ke asc";
		//QUERY
		if(!checkOriSQL($conn['ori']['kapal'],$getPkkbInfo,$query_pkkb_info,$err)) goto Err;
		//FETCH QUERY
		while ($row_pkkb_info = oci_fetch_array($query_pkkb_info, OCI_ASSOC))
		{
			...
			if($row_pkkb_info[LABUH]=='1')
			{
				...
				//QUERY
				if(!checkOriSQL($conn['ori']['kapal'],$getServiceInfo,$query_service_info,$err)) goto Err;
				//FETCH QUERY
