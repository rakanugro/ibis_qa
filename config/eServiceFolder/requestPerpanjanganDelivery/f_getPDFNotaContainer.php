...
function getPDFNotaContainer($in_param) {
	...
	try {
		...
		if($port_code == 'IDJKT' && $terminal_code == 'T009D'){
			$terminal_name = 'TERMINAL OPERASI I';
		}
		...
		$query="select TO_CHAR(DATE_PAID,'DD-MON-YYYY') AS TGL_PROSES,a.NO_FAKTUR_PAJAK, A.NO_NOTA AS KD_UPER, a.NO_REQUEST AS KD_PERMINTAAN, 		A.CUST_NAME NAMA_PELANGGAN, A.CUST_ADDR ALAMAT_PELANGGAN, 
		 ...
		 where d.KD_UPER=a.NO_NOTA AND d.KD_PERMINTAAN = a.NO_REQUEST AND URAIAN='ADM'),0),'999,999,999,999.00') ADM 
		 from tth_nota_all2 a WHERE a.NO_REQUEST='$no_request'";
		 
		if($port_code == 'IDJKT' && $terminal_code == 'ITOST'){
			if(!checkOriSQL($conn['ori']['billing_idjkt_itost'],$query,$query_header,$err)) goto Err;
		} else if ($port_code == 'IDJKT' && $terminal_code == 'T009D'){
			if(!checkOriSQL($conn['ori']['billing_idjkt_t009d'],$query,$query_header,$err)) goto Err;
		}
		$data = oci_fetch_array($query_header, OCI_ASSOC);
		
		
		$date=date('d M Y H:i:s');
 
		$query_dtl="select URAIAN,TGL_AWAL,TGL_AKHIR,QTY, SIZE_,TYPE_,STATUS_,HZ, TOTHARI, TO_CHAR(TARIF,'999,999,999,999.00') TARIF, TO_CHAR(TOTTARIF,'999,999,999,999.00') TOTTARIF from ttR_nota_all WHERE KD_UPER='".$data[NO_FAKTUR_PAJAK]."'  AND KD_PERMINTAAN = '$no_request' AND URAIAN<>'ADM' ORDER BY LINE_NUMBER";
		if($port_code == 'IDJKT' && $terminal_code == 'ITOST'){
			if(!checkOriSQL($conn['ori']['billing_idjkt_itost'],$query_dtl,$queryDetail,$err)) goto Err;
		} else if ($port_code == 'IDJKT' && $terminal_code == 'T009D'){
			if(!checkOriSQL($conn['ori']['billing_idjkt_t009d'],$query_dtl,$queryDetail,$err)) goto Err;
		}	
...
EOD;
		...
		$data = array(
						"nota_html" => base64_encode($html_tcpdf),
