...
function getCountContainer($in_param) {
	...
	try {
		...
		if (($port_code=="IDJKT"&&$terminal_code=="T009D")){
			$qx = "select count(1) as QTY from req_receiving_d where id_req='$biller_request_id'";
		} else {
			$qx = "select count(1) as QTY from req_receiving_d where no_req_anne='$biller_request_id'";
		}
