...
function submitRequestBatalmuat($in_param) {
	...
	try {
		...
		$uname_eservice = $xml_data->data->detail->uname_eservice;
		$tgl_delivery = $xml_data->data->detail->tgl_delivery;
		$nosuratjalan = $xml_data->data->detail->nosuratjalan;
		$nospp = $xml_data->data->detail->nospp;
		$vesselcode = $xml_data->data->detail->vesselcode;
		$callsign = $xml_data->data->detail->callsign;
		...
		//memasukkan data ke tabel header
			...
			if ($terminal_code == 'T3I'){
				...
				$sql_vc = "INSERT INTO REQ_BATALMUAT_H 
					(ID_REQ_OL,
					 KODE_PBM, 
					 JENIS, 
					 VESSEL, 
					 ...
					 NO_SPP, 
					 NO_SRT_JLN)
				VALUES ('$request_no','$customer_id','$tipe', '$vessel', '$voyage_in', '$voyage_out','$shipping_line', '$uname_phd', 'N', TO_DATE('$tgl','dd/mm/rrrr hh24:mi:ss'), '$customer_name', '$cust_alamat', '$cust_npwp', '','','','$booknum','$fpod','$id_fpod','I','$vesselcode','$callsign','009','$nospp','$nosuratjalan')";
				...
				if(!checkOriSQL($conn['ori']['billing'],$q_insert,$q_insert_,$err)) goto Err;
				*/
			
				try {
					if(!checkOriSQL($conn['ori']['billing'],$sql_vc,$sql_vc_,$err)) goto Err; 
					if(!checkOriSQL($conn['ori']['billing'],$sql_h,$sql_h_,$err)) goto Err;
					
					if ($terminal_code == 'T3I'){
						$query_getreq = "SELECT TRIM(MAX(ID_BATALMUAT)) ID_BATALMUAT FROM TB_BATALMUAT_H WHERE KODE_PBM ='$customer_id' AND JENIS = '$tipe' AND voyage = '$voyage_in'";
					} else if ($terminal_code == 'T009D'){
						$query_getreq = "SELECT TRIM(MAX(ID_REQ)) ID_BATALMUAT FROM REQ_BATALMUAT_H WHERE KODE_PBM ='$customer_id' AND JENIS = '$tipe'";
					} else {
					...
					if(!checkOriSQL($conn['ori']['billing'],$query_getreq,$getNoRequest,$err,$bind_param)) goto Err;
					...
					$query = "insert into transaction_log (REQUEST_ID,BILLER_REQUEST_ID, KODE_MODUL,MODUL_DESC,PORT_ID,TERMINAL_ID,STATUS_REQ
															...
															NO_UKK, VESSEL, VOYAGE_IN, VOYAGE_OUT, NO_SPP, NO_SURATJALAN, LAST_USER_ACTIVITY_USERID
															)
					values
					(TRIM('$request_no'), TRIM('$biller_request_no'), 'PTKM08','$tipebm','$port_code', '$terminal_code', 'N','$customer_id', '$customer_name', '$cust_alamat', '$cust_npwp','$cusnumber1', '$cusnumber2', '','$booknum' ,'$fpod' ,SYSDATE, '$uname_eservice', '' , '', '' ,'','', '', '', '','$no_ukk_baru','$vessel','$voyage_in','$voyage_out','$nospp','$nosuratjalan','$uname_eservice' 
					)";
