...
function ePaymentInquiry($notanumber, $userid) {
  ...
  if($kodeperusahaan=='013'){
	...
	$schema_host='10.10.32.25/to3';
  }
  else if($kodeperusahaan=='012'){
	$schema_name='itos_billing';
	$schema_pwd='itos_BILLING';
	$schema_host='10.10.32.25/to2';
  }
  else if($kodeperusahaan=='011'){
	$schema_name='itos_billing';
	$schema_pwd='itos_BILLING';
	$schema_host='10.10.32.25/to1';
  }
  else
  {
	  $error = true;
  ...
  if($error){
	...
	$xml_str = "^92^Nota Tidak Dikenali";
	
	return $xml_str;	  
  }
  else if ($conn = oci_connect($schema_name, $schema_pwd, $schema_host)) 
  {
...
function getReconcilePTPt1($start,$end,$paid_channel,$userid="",$bankid="",$orgid="") {
   ...
   // db connection
   //if  ($conn = oci_connect('billing_to3_dom', 'billingto3', '192.168.23.44/ORCL')) 
   if ($conn = oci_connect('itos_billing', 'itos_BILLING', '10.10.32.25/to1')) 
   {
...
function getReconcilePTPt2($start,$end,$paid_channel,$userid="",$bankid="",$orgid="") {
   ...
   // db connection
   //if  ($conn = oci_connect('billing_to3_dom', 'billingto3', '192.168.23.44/ORCL')) 
   if ($conn = oci_connect('itos_billing', 'itos_BILLING', '10.10.32.25/to2')) 
   {
...
function getReconcilePTPt3($start,$end,$paid_channel,$userid="",$bankid="",$orgid="") {
   ...
   // db connection
   //if  ($conn = oci_connect('billing_to3_dom', 'billingto3', '192.168.23.44/ORCL')) 
   if ($conn = oci_connect('itos_billing', 'itos_BILLING', '10.10.32.25/to3')) 
   {
...
function ePaymentJKMPTP ($in_trx_number,$in_bank_account_id,$in_amount,$in_receipt_date,$in_receipt_category,$in_receipt_source,$in_receipt_comment, $kd_modul) {
	//create log
	//$conn1 = oci_connect('billing_to3_dom', 'billingto3', '192.168.23.44/ORCL');
	$kodeperusahaan=substr($in_trx_number,3,3);
	  if($kodeperusahaan=='013'){
		$schema_name='itos_billing';
		$schema_pwd='itos_BILLING';
		$schema_host='10.10.12.204/to3';
	  }
	  else if($kodeperusahaan=='012'){
		$schema_name='itos_billing';
		$schema_pwd='itos_BILLING';
		$schema_host='10.10.12.204/to2';
	  }
	  else if($kodeperusahaan=='011'){
		$schema_name='itos_billing';
		$schema_pwd='itos_BILLING';
		$schema_host='10.10.12.204/to1';
	  }
