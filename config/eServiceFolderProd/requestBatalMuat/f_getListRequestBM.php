...
function getListRequestBM($in_param) {
	...
	try {
		...
		$getrequestlist = "SELECT BILLER_REQUEST_ID,PORT_ID, TERMINAL_ID,REQUEST_ID, CASE WHEN MODUL_DESC = 'CALBG' THEN 'BEFORE GATEIN' 
					...
					WHEN MODUL_DESC = 'CALDG' THEN 'DELIVERY' END TIPEBM,STATUS_REQ, VESSEL, VOYAGE_IN, VOYAGE_OUT, STATUS_REQ, REQUEST_DATE TGL_REQ, REJECT_NOTES FROM TRANSACTION_LOG WHERE KODE_MODUL = 'PTKM08' and customer_id = '$agent_id' and status_REQ in ('','N','R') ORDER BY REQUEST_DATE DESC";
		//return $getContlist;
