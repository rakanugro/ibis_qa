...
function getListContainerBM($in_param) {
	...
	try {
		...
		if ($terminal_code == 'T009D'){
			$getContlist = " SELECT NO_CONTAINER, JNS_CONT,HZ,ID_CONT, TO_CHAR(TGL_STACK,'dd/mm/rrrr') TGL_STACK,TO_CHAR(TGL_BERANGKAT,'dd/mm/rrrr') TGL_BERANGKAT FROM REQ_BATALMUAT_D WHERE ID_REQ = '$norequest'";
		} else {
		...
		//return $getContlist;
		if(!checkOriSQL($conn['ori']['billing'],$getContlist,$queryContlist,$err)) goto Err;
			//FETCH QUERY
