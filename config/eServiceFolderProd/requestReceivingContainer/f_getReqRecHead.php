...
function getRequestRecHead($in_param) {
	...
	try {
		...
		if(!checkOriSQL($conn['ori']['billing'] ,$query,$query_request,$err)) goto Err; 
		//FETCH QUERY
		...
		$getCarr = "SELECT CDG_OPER_CODE AS CODE, CDG_OPER_NAME AS LINE_OPERATOR
			 ...
			 WHERE CDG_OPER_CODE IN (SELECT VSB_VOYO_OPER 
										  ...
										  WHERE TRIM (UPPER (B.VESSEL)) = TRIM (UPPER ('".
				$request[0]['vessel']
			  ."')))";
		...
		//array_push($infos, 'dummy');
		
		$out_data = array();
		$out_data['request']=$request;
		$out_data['carrier_list']=$infos;
...
function getRequestRecDet($in_param) {
	...
	try {
		...
		if(!checkOriSQL($conn['billing'][0] ,$query,$query_request,$err)) goto Err; 
		//FETCH QUERY
