...
function getThroughput($in_param) {
	...
	try {
		...
		//QUERY
		if(!checkOriSQL($conn['container'][0],$sql[0],$query_container_e,$err)) goto Err;
		if(!checkOriSQL($conn['container'][0],$sql[1],$query_container_i,$err)) goto Err;
		if(!checkOriSQL($conn['container'][0],$sql[2],$query_container_t,$err)) goto Err;
