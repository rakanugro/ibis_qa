...
/*|
 ...
 | EDIT LOG			: 
 |*/
function getRequestDeliveryExt($in_param) {
	try {
		$request = array();
		...
		{
			$getRequest = "SELECT ID_REQ , OLD_REQ, 
			...
			$getRequest = "SELECT a.ID_REQ , OLD_REQ, 
		...
		else		
		{
			$getRequest = "SELECT ID_REQ , OLD_REQ, 
							...
							TO_CHAR(TGL_SPPB,'dd-mm-yyyy') DATE_SPPB , SP_CUSTOM NO_SP_CUSTOM , 
							TO_CHAR(TGL_SP_CUSTOM,'dd-mm-yyyy') DATE_SP_CUSTOM , BL_NUMB NO_BL, 
							TO_CHAR(TGL_SP2,'dd-mm-yyyy') DATE_DELIVERY,TO_CHAR(TGL_EXT,'dd-mm-yyyy') DATE_EXT, TO_CHAR(DISCH_DATE,'dd-mm-yyyy') DATE_DISCHARGE , 
