...
/*|
 | Function Name 	: getListRestitutionCont
 | Description 		: Get List Container
 |*/
function getListRestitutionCont($in_param) {
	...
	try {
		...
		if($pol=='IDJKT' && $id_joint_vessel=='009X'){
			//gate
			$qCoco="SELECT A.NO_CONTAINER, to_char(to_date(TRUCK_OUT_DATE,'rrrrmmddhh24miss'),'dd/mm/yyyy hh24:mi:ss') TRUCK_OUT_DATE, to_char(to_date(DISCHARGE_CONFIRM,'rrrrmmddhh24miss'),'dd/mm/yyyy hh24:mi:ss') DISCHARGE_CONFIRM
					  FROM    SBY_COARRI A
						   LEFT JOIN
							  SBY_CODECO B
			...
			{	
				...
				*/
				...
				//ambil codeco
				$truck_in_date = null;
				$qContainer = "select no_container,to_char(to_date(TRUCK_IN_DATE,'rrrrmmddhh24miss'),'dd/mm/rrrr hh24:mi:ss') TRUCK_IN_DATE from SIMPB.CODECO_PTP WHERE VESSEL = '$vessel_name' $filterin $filterout ";
				//QUERY
				try {
				...
				$qContainer = "/* Formatted on 3/18/2015 9:08:12 AM (QP5 v5.163.1008.3004) */
								select no_container,status,iso_code,imo,carrier,to_char(to_date(loading_confirm,'rrrrmmddhh24miss'),'dd/mm/rrrr hh24:mi:ss') loading_confirm from simpb.coarri_ptp
								 WHERE VESSEL = '$vessel_name' $filterin $filterout";
				//QUERY
				try {
					if(!checkOriSQL($conn['container'][$j],$qContainer,$getContainer,$err,$debug)) goto Err;
					//FETCH QUERY
			...
			{	
				...
				*/
				...
				{
					//build "info" data
					if($row_container[GATE_IN_DATE] != NULL && $row_container[VESSEL_CONFIRM] != NULL && $discharge[$row_container[NO_CONTAINER]] != NULL && $truck_out[$row_container[NO_CONTAINER]] != NULL){
						$qibiscek = "SELECT AKTIF,STATUS_REQ FROM REQ_RESTITUTION_D, REQ_RESTITUTION_H WHERE REQ_RESTITUTION_D.ID_REQ = REQ_RESTITUTION_H.ID_REQ AND NO_CONTAINER = '".$row_container[NO_CONTAINER]."' AND CALL_SIGN = '$call_sign'
						AND VOYAGE_IN = '$voyage_in' AND VOYAGE_OUT = '$voyage_out'";
						if(!checkOriSQL($conn['ori']['ibis'],$qibiscek,$qibiscek_,$err,$debug)) goto Err;
						$row_status = oci_fetch_array($qibiscek_, OCI_ASSOC);
						if(($row_status[AKTIF] == 'Y' && $row_status[STATUS_REQ] == 'S') || ($row_status[AKTIF] == 'T' && $row_status[STATUS_REQ] == 'R')){
							$nilai = 2;
						}
						else {
							$nilai = 1;
					...
					$qinsert = "INSERT INTO REQ_RESTITUTION_D(ID_REQ, NO_CONTAINER, SIZE_, TYPE_, STATUS_, GATE_IN, LOAD_DATE, DISCH_DATE, GATE_OUT, AKTIF)
					VALUES ('$id_req','".$row_container[NO_CONTAINER]."','".$row_container[SIZE_]."','".$row_container[ISO_CODE]."'
					,'".$row_container[STATUS]."',TO_DATE('".$row_container[GATE_IN_DATE]."','DD-MM-YYYY HH24:MI:SS'),
					TO_DATE('".$row_container[VESSEL_CONFIRM]."','DD-MM-YYYY HH24:MI:SS')
					,TO_DATE('".$discharge[$row_container[NO_CONTAINER]]."','DD-MM-YYYY HH24:MI:SS'),
					TO_DATE('".$truck_out[$row_container[NO_CONTAINER]]."','DD-MM-YYYY HH24:MI:SS'),'T')";
					if(!checkOriSQL($conn['ori']['ibis'],$qinsert,$qinsert_,$err,$debug)) goto Err;
				}
			}
		

		}/*  else {
			$qCoco="SELECT A.NO_CONTAINER,  to_char(to_date(TRUCK_IN_DATE,'rrrrmmddhh24miss'),'dd/mm/yyyy hh24:mi:ss') TRUCK_IN_DATE, to_char(to_date(LOADING_CONFIRM,'rrrrmmddhh24miss'),'dd/mm/yyyy hh24:mi:ss') LOADING_CONFIRM
						  FROM    SBY_CODECO A
							   LEFT JOIN
								  SBY_COARRI B
							   ON (A.ID_JOINT_VESSEL = B.ID_JOINT_VESSEL
			...
			for($j=0;$j<count($conn['container']);$j++)
			{	
				$qContainer="SELECT NO_CONTAINER, STATUS, ISO_CODE, IMO, CARRIER,  to_char(to_date(GATE_OUT_DATE,'rrrrmmddhh24miss'),'dd/mm/yyyy hh24:mi:ss') GATE_OUT_DATE, to_char(to_date(VESSEL_CONFIRM,'rrrrmmddhh24miss'),'dd/mm/yyyy hh24:mi:ss') VESSEL_CONFIRM FROM M_CYC_CONTAINER WHERE POL='IDSUB' AND E_I='I' AND VESSEL='$vessel_name' AND VOYAGE_IN='$voyage_in' AND VOYAGE_OUT='$voyage_out'";
				//QUERY
				if(!checkOriSQL($conn['container'][$j],$qContainer,$getContainer,$err,$debug)) goto Err;
				//FETCH QUERY
				while ($row_container = oci_fetch_array($getContainer, OCI_ASSOC))
