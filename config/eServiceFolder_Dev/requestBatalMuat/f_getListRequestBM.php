...
/*|
 ...
 | EDIT LOG			:   
 |*/
function getListRequestBM($in_param) {
	...
	try {
		...
		$getrequestlist = "SELECT BILLER_REQUEST_ID,PORT_ID, TERMINAL_ID,REQUEST_ID, CASE WHEN MODUL_DESC = 'CALBG' THEN 'BEFORE GATEIN' 
					WHEN MODUL_DESC = 'CALAG' THEN 'AFTER GATEIN' 
					WHEN MODUL_DESC = 'CALDG' THEN 'DELIVERY' END TIPEBM,STATUS_REQ, VESSEL, VOYAGE_IN, VOYAGE_OUT, STATUS_REQ, TO_CHAR(REQUEST_DATE,'DD/MM/RRRR HH24:MI:SS') TGL_REQ FROM TRANSACTION_LOG WHERE KODE_MODUL = 'PTKM08' ORDER BY TGL_REQ DESC";
		//return $getContlist;
		if(!checkOriSQL($conn['ori']['ibis'],$getrequestlist,$requestlist,$err,$debug)) goto Err;
			//FETCH QUERY
		...
		while ($row_req = oci_fetch_array($requestlist, OCI_ASSOC))
			{
				...
				//return $getqty;
				if(!checkOriSQL($conn['billing'][0],$queryqty,$getqty,$err,$debug)) goto Err;
