...
function savePaymentConfirmation($in_param) {
	...
	try {
		...
		if(!checkOriSQL($conn['ori']['billing_idjkt_itost'],$query_payment_nbs,$paymentNBS,$err,$debug)) goto Err;
		...
		if($v_status=="P"){
			...
			if(!checkOriSQL($conn['ori']['ibis'],$query_save_payment,$save_payment,$err,$debug)) goto Err;
