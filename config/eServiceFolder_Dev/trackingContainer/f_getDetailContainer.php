...
function getDetailContainer($in_param) {
	...
	try {
		...
		else if($port_code=="IDJKT")
		{
			...
			$conn['billing'][4] = $conn['ori']['billing_idjkt_t009d'];
		}
		...
		else if($port_code==""&&$terminal_code=="")
		{
			...
			$conn['billing'][2] = $conn['ori']['billing_idjkt_t2d'];
			$conn['billing'][3] = $conn['ori']['billing_idjkt_t1d'];
			$conn['billing'][4] = $conn['ori']['billing_idpnk_t3i'];
		...
		$query_handling="SELECT ACTIVITY AS KEGIATAN,
						 E_I,
		...
		for($j=0;$j<count($conn['container']);$j++)
		{
			if(!checkOriSQL($conn['container'][$j],$query_handling,$getHandling,$err,$debug)) goto Err;
			//FETCH QUERY
						   ...
						   FROM req_receiving_d
								...
								LEFT JOIN req_receiving_h
								   ON (TRIM (req_receiving_h.ID_REQ) =
										  TRIM (req_receiving_d.NO_REQ_ANNE))
						   WHERE req_receiving_h.NO_UKK = '".$no_ukk."'
						  UNION                                                                                   
						  SELECT NO_CONTAINER,
		...
		{
			if(!checkOriSQL($conn['billing'][$j],$sq,$getBilling,$err,$debug)) goto Err;
			//FETCH QUERY
			while ($row_billing = oci_fetch_array($getBilling, OCI_ASSOC))
			{
				$billing_tmp = array(
						...
						'no_invoice' => $row_billing[ID_NOTA],
