...
function requestReceivingDetail($in_param) {
	...
	try {
		...
		$port = $xml_data->data->detail->port;$port = explode("-",$port);
		$port_code=$port[0];
		...
		//insert data
		//PL/SQL //biller
		$cek_iso = "select m.ISO_CODE from (select ISO_CODE from master_iso_code WHERE SIZE_='$size' AND TYPE_='$type' AND H_ISO='$height') m where rownum=1";
		if(!checkOriSQL($conn['billing'][0],$cek_iso,$query_iso,$err,$debug)) goto Err;
		...
		if($port_code=="IDJKT"&&$terminal_code=="T3I"){
			$param_b_var = array(
					...
					"v_nor" => "N",
					"v_jns_keg" => "$tl_type",
					"v_oogs" => "", //ask dama atau ardi perubahan parameter yg ada di procedure yg baru by gandul 3 Aug 2015 17:40 PM
					"v_berat" => "$weight", //ask dama atau ardi perubahan parameter yg ada di procedure yg baru by gandul 3 Aug 2015 17:40 PM
					":v_msg" => ""
				);

			$query_repo_anne = "declare begin proc_add_cont_anne(:v_nc,:v_req,:v_sc,:v_tc,:v_stc,:v_hc,:v_comm,:v_imo,:v_iso,:v_book,:v_hgc,:v_ship,:v_car,:v_tmp,:v_oh, :v_ow, :v_ol,:v_un,:v_nor,:v_berat,:v_msg); end;";
		} 
		else if ($port_code=="IDJKT"&&$terminal_code=="T009D"){
		...
		if ($port_code=="IDJKT"&&$terminal_code=="T009D"){
			...
			if(!checkOriSQL($conn['billing'][0],$query_cek_limit,$cek_limit,$err,$debug)) goto Err;
			...
			if(!checkOriSQL($conn['billing'][0],$query_cek_row,$cek_row,$err,$debug)) goto Err;
			...
			if(($cont_limit > $jumlah_row) or empty($cont_limit)){
				...
				if(!checkOriSQL($conn['billing'][0],$query_repo_anne,$queryrepo,$err,$debug,$param_b_var)) goto Err;
