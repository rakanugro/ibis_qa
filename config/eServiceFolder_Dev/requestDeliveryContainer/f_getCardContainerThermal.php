...
function getCardContainerThermal($in_param) {
	...
	try {
		...
		if(!checkOriSQL($conn['ori']['billing_idjkt_itost'],$query_check,$queryCheck,$err,$debug)) goto Err;
		...
		if(!checkOriSQL($conn['ori']['container_idjkt_itost'],$row,$queryVessel,$err,$debug)) goto Err;;
	...
	if(!checkOriSQL($conn['ori']['billing_idjkt_itost'],$rw,$rW,$err,$debug)) goto Err;
	//FETCH QUERY
	$nourut=0;
