...
class payment_cash extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('npksbilling/billing_management/payment_cash_model');
	...
	public function index()
	{
		...
		$data['title'] = "Payment";
		$this->common_loader($data, 'npksbilling/billing_management_npks/payment_cash');
	}

	public function view_payment($id, $nota_paid)
	{
		...
		if ($nota_paid == "Y") {
			$data['detail_payment'] = $this->payment_cash_model->detail_payment_bank($id_data);
		} else {
			$data['detail_payment'] = $this->payment_cash_model->detail_payment($id_data);
		}
		$data['type_data'] = $type_data;
		$this->common_loader($data, 'npksbilling/billing_management_npks/view_payment_cash');
	}

	public function save_payment_cash()
	{
		$json = $this->input->post('data');
		$data = $this->payment_cash_model->save_payment_cash($json);
		echo $data;
	}
	...
	// 	$data['menu_list'] = $this->user_model->get_menuList($this->session->userdata('group_phd'));
	// 	$data['title'] = "Form Payment";
	// 	$data['bank'] = $this->payment_cash_model->getBankList();
	// 	$data['detail_payment'] = $this->payment_cash_model->detail_payment($id_data);
	// 	$data['type_data'] = $type_data;
