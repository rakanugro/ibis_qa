...
//session_start();
class appbtlplug extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model('npksbilling/approval/appbatalplug_model');
	...
	public function index(){
		...
		$data['title']= "Approval Batal Plugging Reefer";

		$this->common_loader($data,'npksbilling/approval/approval_batal_plugging_reefer');
	}

	public function getList()
	{
		$data = $this->appbatalplug_model->getList();
		echo $data;
	}
	...
	{
		$data['menu_list'] = $this->user_model->get_menuList($this->session->userdata('group_phd'));
		$data['title'] = "Preview Approval Plugin Reefer";
		$data['id'] = $id;
		$data['cancel_id'] = $cancel_id;
		$data['docType'] = $this->appbatalplug_model->getDocType();
		$this->common_loader($data, 'npksbilling/approval/preview_batal_pluggin_reefer');
	}

	public function update_plug($id = null)
	{
		if ($id) {
			$data = $this->appbatalplug_model->update_plug($id);
			echo $data;
		} else {
			$arr = array(
				"HEADER" => ""
	...
	{
		$data = $this->appbatalplug_model->getTarif($id);
		echo json_encode($data);
	}

	public function approve($id)
	{
		$data = $this->appbatalplug_model->approve($id);
		echo json_encode($data);
	}
	...
	public function reject($id)
	{
		...
		$data = $this->appbatalplug_model->reject($id, $remarks);
		echo json_encode($data);
	}
