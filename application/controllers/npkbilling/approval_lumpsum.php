...
class approval_lumpsum extends CI_Controller {
	...
	public function index(){
		...
		$this->breadcrumbs->push("Approval lumpsum", '/');
		$this->breadcrumbs->unshift('Home', '/');
		$data['breadcrumbs'] = $this->breadcrumbs->show();

		$data['title']= "Approval lumpsum";

		$this->common_loader($data,'npkbilling/approval/approval_lumpsum');
	}

	public function preview_approval_lumpsum()
	{
		...
		$data['title']= "Preview Approval lumpsum";

		$this->common_loader($data,'npkbilling/approval/preview_approval_lumpsum');
	...
	public function create_lumpsum()
	{
		...
		$this->breadcrumbs->push("lumpsum Booking", 'container_lumpsum/main_lumpsum');
		$this->breadcrumbs->push("lumpsum Booking", '/');
		$this->breadcrumbs->unshift('Home', '/');
		$data['breadcrumbs'] = $this->breadcrumbs->show();

		$data['title']= "Create Request lumpsum";

		$this->common_loader($data,'npkbilling/booking/create_lumpsum');
	}

	public function getListApprovallumpsum()
      ...
      <ipc:indexService>
         <indexServiceInterfaceRequest>
            ...
            [
            {
                ...
                "field1" : "TX_HDR_LUMPSUM.LUMPS_STATUS"
            }
            ], 
            "where"      : [["TM_REFF.REFF_TR_ID", "=", "8"], ["TX_HDR_LUMPSUM.LUMPS_STATUS", "=", "2"], ["TX_HDR_LUMPSUM.APP_ID", "=", "2"]],
            "whereIn"    : [],
            "whereIn2"   : [],
            "whereNotIn" : [],
            "range"      : [],
            "select"     : [],
            "orderby"    : ["TX_HDR_LUMPSUM.LUMPS_ID", "DESC"],
            "changeKey"  : ["",""],
            "query"      : "",
            "field"      : "",
            "limit"      : 25,
             ...
             <request>
{
  ...
  "FILE": {
   ...
   <soapenv:Body>
      <ipc:storeService>
         <storeServiceInterfaceRequest>
            ...
            <esbBody>
               <request>{"action":"update","db":"omcargo","table":"TX_HDR_LUMPSUM","update":{"LUMPS_STATUS":"3"},"where":{"LUMPS_ID":"'.$id.'"}}</request>
            </esbBody>
            <esbSecurity>
               <orgId>?</orgId>
               <batchSourceId>?</batchSourceId>
   ...
   <soapenv:Body>
      <ipc:storeService>
         <storeServiceInterfaceRequest>
            ...
            <esbBody>
               <request>{"action":"update","db":"omcargo","table":"TX_HDR_LUMPSUM","update":{"LUMPS_STATUS":"4", "LUMPS_MARK": "'.$remarks.'"},"where":{"LUMPS_ID":"'.$id.'"}}</request>
            </esbBody>
            <esbSecurity>
               <orgId>?</orgId>
               <batchSourceId>?</batchSourceId>
	...
	public function getListTerminal() {
		...
		$result = $this->nusoap_lib->SendCurl($xml, $wsdl, 'terminalList', 'npk_billing', 'npk_billing');
	...
	public function auto_vessel(){
		...
		$result = $this->nusoap_lib->SendCurl($xml, $wsdl, 'trackingVessel', 'npk_billing', 'npk_billing');
	...
	public function auto_no_peb(){
		...
		$result = $this->nusoap_lib->SendCurl($xml, $wsdl, 'searchPEB', 'npk_billing', 'npk_billing');
		if(!$result){
			echo $result;
		}else{
			$response = $this->xml2array->xml2ary($result['response']);
   ...
   <soapenv:Header/>
   <soapenv:Body>
      <ipc:getListApprovallumpsum>
         <getListApprovallumpsumInterfaceRequest>
            <esbHeader>
               <!--Optional:-->
               <internalId>?</internalId>
               <!--Optional:-->
            ...
            <esbBody>
               <booking_type>lumpsum</booking_type>
			...
			$result = $this->nusoap_lib->SendCurl($xml, $wsdl, 'getListApprovallumpsum', 'npk_billing', 'npk_billing');
		...
		if(!$result){
			...
			$response = $this->xml2array->xml2ary($result['response']);
			$out_param = $response['soapenv:Envelope']['_c']['soapenv:Body']['_c']['ser-root:getListApprovallumpsumResponse']['_c']['getListApprovallumpsumInterfaceResponse']['_c']['esbBody']['_c']['results'];
			//var_dump($response);
			$aa = json_encode($out_param);
			echo $aa;
		}
