...
class approval_receiving extends CI_Controller {
	...
	public function index(){
		...
		$this->breadcrumbs->push("Approval Receiving", '/');
		$this->breadcrumbs->unshift('Home', '/');
		$data['breadcrumbs'] = $this->breadcrumbs->show();

		$data['title']= "Approval Receiving";

		$this->common_loader($data,'npkbilling/approval/approval_receiving');
	}

	public function preview_approval_receiving()
	{

		$data['id_param'] = $this->input->get('id');
		$data['menu_list']=$this->user_model->get_menuList($this->session->userdata('group_phd'));
		
		$this->breadcrumbs->push("Preview Approval Receiving", 'container_receiving/main_receiving');
		$this->breadcrumbs->unshift('approval receiving ', '/');
		$data['breadcrumbs'] = $this->breadcrumbs->show();

		$data['title']= "Preview Approval Receiving";

		$this->common_loader($data,'npkbilling/approval/preview_approval_receiving');
	}
	...
	public function create_receiving()
	{
		...
		$this->common_loader($data,'npkbilling/booking/create_receiving');
	}

	public function getListApprovalReceiving()
   ...
   <soapenv:Body>
      <ipc:indexService>
         <indexServiceInterfaceRequest>
            ...
            "where"      : [["TM_REFF.REFF_TR_ID", "=", "8"], ["TX_HDR_REC.REC_STATUS", "=", "2"], ["TX_HDR_REC.APP_ID", "=", "2"]],
            "whereIn"    : [],
            "whereIn2"   : [],
            "whereNotIn" : [],
            "range"      : [],
             ...
             <request>
{
  ...
  "FILE": {
   ...
   <soapenv:Header/>
   <soapenv:Body>
      <ipc:getListApprovalReceiving>
         <getListApprovalReceivingInterfaceRequest>
            <esbHeader>
               <!--Optional:-->
               <internalId>?</internalId>
               <!--Optional:-->
            ...
            <esbBody>
               <booking_type>receiving</booking_type>
			...
			$result = $this->nusoap_lib->SendCurl($xml, $wsdl, 'getListApprovalReceiving', 'npk_billing', 'npk_billing');
		...
		if(!$result){
			...
			$response = $this->xml2array->xml2ary($result['response']);
			$out_param = $response['soapenv:Envelope']['_c']['soapenv:Body']['_c']['ser-root:getListApprovalReceivingResponse']['_c']['getListApprovalReceivingInterfaceResponse']['_c']['esbBody']['_c']['results'];
			//var_dump($response);
			$aa = json_encode($out_param);
			echo $aa;
		}
