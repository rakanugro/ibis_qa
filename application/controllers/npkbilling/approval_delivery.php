...
class approval_delivery extends CI_Controller {
	...
	public function index(){
		...
		$this->breadcrumbs->push("Approval delivery", '/');
		$this->breadcrumbs->unshift('Home', '/');
		$data['breadcrumbs'] = $this->breadcrumbs->show();

		$data['title']= "Approval delivery";

		$this->common_loader($data,'npkbilling/approval/approval_delivery');
	}

	public function preview_approval_delivery()
	{
		...
		$data['title']= "Preview Approval delivery";

		$this->common_loader($data,'npkbilling/approval/preview_approval_delivery');
	}
	...
	public function create_delivery()
	{
		...
		$this->breadcrumbs->push("delivery Booking", 'container_delivery/main_delivery');
		$this->breadcrumbs->push("delivery Booking", '/');
		$this->breadcrumbs->unshift('Home', '/');
		$data['breadcrumbs'] = $this->breadcrumbs->show();

		$data['title']= "Create Request delivery";

		$this->common_loader($data,'npkbilling/booking/create_delivery');
	}

	public function getListApprovaldelivery()
   ...
   <soapenv:Body>
      <ipc:indexService>
         <indexServiceInterfaceRequest>
            ...
            "where"      : [["TM_REFF.REFF_TR_ID", "=", "8"], ["TX_HDR_DEL.DEL_STATUS", "=", "2"], ["TX_HDR_DEL.DEL_EXT_LOOP", "=", "1"], ["TX_HDR_DEL.APP_ID", "=", "2"]],
            "whereIn"    : [],
            "whereIn2"   : [],
            "whereNotIn" : [],
            "range"      : [],
             ...
             <request>
{
  ...
  $wsdl = NPK_WSDL;
   ...
   <soapenv:Header/>
   <soapenv:Body>
      <ipc:getListApprovaldelivery>
         <getListApprovaldeliveryInterfaceRequest>
            <esbHeader>
               <!--Optional:-->
               <internalId>?</internalId>
               <!--Optional:-->
			...
			$result = $this->nusoap_lib->SendCurl($xml, $wsdl, 'getListApprovaldelivery', 'npk_billing', 'npk_billing');
		...
		if(!$result){
			...
			$response = $this->xml2array->xml2ary($result['response']);
			$out_param = $response['soapenv:Envelope']['_c']['soapenv:Body']['_c']['ser-root:getListApprovaldeliveryResponse']['_c']['getListApprovaldeliveryInterfaceResponse']['_c']['esbBody']['_c']['results'];
			//var_dump($response);
			$aa = json_encode($out_param);
			echo $aa;
		}
