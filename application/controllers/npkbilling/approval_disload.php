...
class approval_disload extends CI_Controller {
	...
	public function index(){
		...
		$this->breadcrumbs->push("Approval Bongkar Muat", '/');
		$this->breadcrumbs->unshift('Home', '/');
		$data['breadcrumbs'] = $this->breadcrumbs->show();

		$data['title']= "Approval Bongkar/Muat";

		$this->common_loader($data,'npkbilling/approval/approval_disload');
	}

	public function preview_approval_disload()
	{
		$data['id_param'] = $this->input->get('id');
		$data['menu_list']=$this->user_model->get_menuList($this->session->userdata('group_phd'));

		$this->breadcrumbs->push("Approval disload", '/');
		$this->breadcrumbs->unshift('Home', '/');
		$data['breadcrumbs'] = $this->breadcrumbs->show();

		$data['title']= "Preview Approval Bongkar/Muat";

		$this->common_loader($data,'npkbilling/approval/preview_approval_disload');
	...
	public function getListApprovaldisload()
						...
						<ipc:indexService>
							<indexServiceInterfaceRequest>
								...
								[
								{
									...
									"field1" : "TX_HDR_BM.BM_STATUS"
								}
								], 
								"where"      : [["TM_REFF.REFF_TR_ID", "=", "8"], ["TX_HDR_BM.BM_STATUS", "=", "2"], ["TX_HDR_BM.APP_ID", "=", "2"]],
								"whereIn"    : [],
								"whereIn2"   : [],
								"whereNotIn" : [],
								"range"      : [],
								"select"     : [],
								"orderby"    : ["TX_HDR_BM.BM_ID", "DESC"],
								"changeKey"  : ["",""],
								"query"      : "",
								"field"      : "",
								"limit"      : 25,
   ...
   <soapenv:Body>
      <ipc:getListApprovaldisload>
         <getListApprovaldisloadInterfaceRequest>
			...
			$result = $this->nusoap_lib->SendCurl($xml, $wsdl, 'getListApprovaldisload', 'npk_billing', 'npk_billing');
