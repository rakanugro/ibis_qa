...
class administrasi extends CI_Controller {
	...
	function masterwilayahsave(){
		$data = ($_POST);
		//print_r($data);
		$result = $this->senddataurl('wilayah',$data,'PUT');
		print_r($result);
	...
	function masterwilayah(){
		$data['prods'] = $this->getdataurl('wilayah');
		$this->common_loader($data,'invoice/administrasi/masterwilayah/masterwilayah');
	}
	...
	function masterwilayahhapus(){
		$data = ($_POST);
		$result = $this->senddataurl('wilayah',$data,'DELETE');
	...
	function masternota(){
		...
		$data['prods'] = $this->getdataurl('nota');
		$this->common_loader($data,'invoice/administrasi/masternota/masternota');
	}

	function masternotasearch(){
		/*$postdata = ($_POST);
		// $data = $this->getdataurl('nota');
		$data = $this->senddataurl('nota/search/',$postdata,'POST');
		// print_r($data);die;
		echo json_encode($data);*/
		$postdata = ($_POST);
		//$postdata = array();
		//foreach($_POST as $req => $param) {
			//$postdata[$req] = strtoupper($param);
		//}
	...
	function masternotaupdate(){
		$data = ($_POST);
		$result = $this->senddataurl('nota',$data,'POST');
		echo json_encode($result);
	}

	function masternotasave(){

		$data = ($_POST);
		$result = $this->senddataurl('nota',$data,'PUT');
		echo json_encode($result);
	}

	function masternotaedit(){
		$postdata = ($_POST);
		$data = $this->senddataurl('nota/search/',$postdata,'POST');
		echo json_encode($data);
	}
	...
	function masterunit(){
		...
		if($this->session->userdata('role_type') == 'Super Admin') {
			...
			$dataArray = array(
				'INV_ENTITY_CODE' => $this->session->userdata('entity_code')
			);
		...
		$data['pjbt'] = $this->getdataurl('pejabat');
		$this->common_loader($data,'invoice/administrasi/masterunit/masterunit');
	}

	function masterunitsave(){
		$data = ($_POST);
		$result = $this->senddataurl('unit',$data,'PUT');
		echo json_encode($result);
	}

	function masterunitupdate(){
		$data = ($_POST);
		$result = $this->senddataurl('unit',$data,'POST');
		echo json_encode($result);

	}

	function masterunitsearch(){
		$postdata = ($_POST);
		$session_unit = $this->session->userdata('unit_id');
		$session_unit = str_replace(array(']', '['), '', $session_unit);
		$session_unit = str_replace('"',"'", $session_unit);
		$postdata['SESSION_UNIT'] = $session_unit;
		$dataArray = $this->senddataurl('unit/search/',$postdata,'POST');
		$data = array(
			'data'=>array()
		);
		$num = 1;
		foreach($dataArray as $key => $value) {
			...
			$data['data'][$key]['num'] = $num;
			$data['data'][$key]['action'] = '<button type="button" onclick="editconfig(\''.$data['data'][$key]['INV_UNIT_ID'].'\')"  class="btn btn-primary btn-sm" data-toggle="modal" data-target="#add_config"> Nota Config</button> | <button type="button"  onclick="editunit(\''.$data['data'][$key]['INV_UNIT_ID'].'\')"  class="btn btn-primary btn-sm" data-toggle="modal" data-target="#update_unit"><i class="fa fa-pencil-square"></i></button></td>';
			$num++;
		}
		echo json_encode($data);
	}

	function masterunitedit(){
		$postdata = ($_POST);
		$id = $postdata['INV_UNIT_ID'];
		$data = $this->getdataurl('unit/'.$id);
		echo json_encode($data);
	}

	function masterpejabat(){
		// $data['pjbt'] = $this->getdataurl('pejabat');
		$this->common_loader($data,'invoice/administrasi/masterunit/masterunit');
	}


	function masterpejabatsearch(){
		$postdata = ($_POST);
		// print_r($postdata);die;
		$data = $this->senddataurl('pejabat/search/',$postdata,'POST');
		$data1 = array(
				'data' => array()
			);
		foreach ($data as $key => $value) {
			$data1['data'][$key] = $value;
			$data1['data'][$key]->INV_PEJABAT_EFECTIVE = date('Y-m-d', strtotime($value->INV_PEJABAT_EFECTIVE));
			$data1['data'][$key]->INV_PEJABAT_EXPIRED = date('Y-m-d', strtotime($value->INV_PEJABAT_EXPIRED));
		}
		echo json_encode($data);
	}

	function masterpejabatedit(){
		$postdata = ($_POST);
		$id = $postdata['INV_PEJABAT_ID'];
		// print_r($id);die;
		$data = $this->getdataurl('pejabat/'.$id);
		echo json_encode($data);
	}
	...
	function masterredaksisave(){
		$data = ($_POST);
		$result = $this->senddataurl('redaksi',$data,'PUT');
		echo json_encode($result);
	}

	function masterredaksisearch(){
		$postdata = ($_POST);
		$dataArray = $this->senddataurl('redaksi/search/',$postdata,'POST');
		$data = array(
			'data'=>array()
		);
		$num = 1;
		foreach($dataArray as $key => $value) {
			...
			$data['data'][$key]['num'] = $num;
			$data['data'][$key]->INV_REDAKSI_EFECTIVE = date('Y-m-d', strtotime($value->INV_REDAKSI_EFECTIVE));
			$data['data'][$key]->INV_PEJABAT_EXPIRED = date('Y-m-d', strtotime($value->INV_PEJABAT_EXPIRED));
			$data['data'][$key]['action'] = '<button type="button" onclick="editredaksi(\''.$data['data'][$key]['INV_REDAKSI_ID'].'\')"  class="btn btn-primary btn-sm" data-toggle="modal" data-target="#update_redaksi">Edit</button>';
			$num++;
		}
		echo json_encode($data);
	}
	function masterredaksiedit(){
		$postdata = ($_POST);
		$id = $postdata['INV_REDAKSI_ID'];
		// print_r($id);die;
		$data = $this->getdataurl('redaksi/'.$id);
		echo json_encode($data);
	}

	function masterredaksiupdate(){
		$data = ($_POST);
		$result = $this->senddataurl('redaksi',$data,'POST');
		echo json_encode($result);

	}

	public function adding_unit(){
		$uri = API_EINVOICE.'/unit';
		$apiKey = '123456';
		$params = array(
			'Content-Type: application/json',
			'x-api-key:'.$apiKey
		...
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($ch, CURLOPT_HTTPHEADER, $params);
		$data['prods']  = json_decode(curl_exec($ch));
		$this->common_loader($data,'invoice/administrasi/masterunit/addmasterunit');
	}


	function masteruser(){
	    // START Penambahan Role Entity di Master Unit by SIGMA 18/12/2019
        if($this->session->userdata('role_type') == 'Super Admin') {
            $data['entity'] = $this->getdataurl('entity');
        } else {
            $dataArray = array(
        ...
        $role_id =  $this->session->userdata('role_id')	;
        $data['layanan'] = $this->auth_model->get_layanan($role_id);
        // STOP Penambahan Role Entity di Master Unit by SIGMA 18/12/2019

		$data['role'] = $this->getdataurl('role');
		
		/****************** 20180824 3ono *************************/
		$data['unit'] = $this->session->userdata('unit_id');
		$searchArray = array('[', ']', '"');
		$replaceArray = array('', '', '');
		$unitArray = str_replace($searchArray, $replaceArray, $this->session->userdata('unit_id'));
		$postdata['INV_UNIT_CODE'] = explode(",", $unitArray);
		
		$data['optrole'] = $this->senddataurl('role/search2/',$postdata,'POST');
		/*********************************************************/
		$this->common_loader($data,'invoice/administrasi/masteruser/masteruser');
	}

	function masterusersearch(){
		$postdata = ($_POST);
		
		$user = $this->senddataurl('user/search/',$postdata,'POST');
		// print_r($data);die;
		//echo json_encode($data);
		...
		$data = array();
		$no = 1;
		$role = array('','Super Admin','Admin Entity','Admin Unit','User');
		foreach ($user as $item) {

			$row = array();
			$row[] = $no++;
			$row[] = htmlspecialchars($item->INV_USER_NAME,ENT_QUOTES);
			$row[] = htmlspecialchars($item->INV_USER_NIPP,ENT_QUOTES);
			$row[] = htmlspecialchars($item->INV_USER_USERNAME,ENT_QUOTES);
			/*$row[] = '*****';*/
			$row[] = htmlspecialchars($item->INV_ROLE_NAME,ENT_QUOTES);
			$row[] = htmlspecialchars($item->INV_USER_STATUS,ENT_QUOTES);


			$row[] = '<a class="btn btn-sm btn-primary"  data-toggle="modal" data-target="#update_user" href="javascript:void(0)" title="Edit" data-backdrop="static" data-keyboard="false" onclick="edituser('."'".$item->INV_USER_ID."'".')"> Edit </a> | <a class="btn btn-sm btn-primary" data-toggle="modal" data-target="#add_config" href="javascript:void(0)" title="Edit" onclick="loaddata2('."'".$item->INV_USER_ID."'".')"> Assign Role </a>';
	...
	function masterusersave(){
		...
		$data = array(
			...
			'INV_USER_STATUS' => $this->input->post('INV_USER_STATUS'),
			'INV_USER_ROLE_ID' => $this->input->post('INV_USER_ROLE_ID'),
			'INV_USER_ENTITY_ID' => $this->input->post('INV_USER_ENTITY_ID')
		);
// print_r($data);	die;
		$result = $this->senddataurl('user',$data,'PUT');
		echo json_encode($result);

	}

	function masteruseredit(){
		$postdata = ($_POST);
	...
	function masteruserupdate(){
		...
		$data = array(
			'INV_USER_ID' => $this->input->post('INV_USER_ID'),
			'INV_USER_NAME' => $this->input->post('INV_USER_NAME'),
			'INV_USER_NIPP' => $this->input->post('INV_USER_NIPP'),
			'INV_USER_USERNAME' => $this->input->post('INV_USER_USERNAME'),
			'INV_USER_PASSWORD' => md5($this->input->post('INV_USER_PASSWORD')),
			...
			'INV_USER_STATUS' => $this->input->post('INV_USER_STATUS'),
			'INV_USER_ROLE_ID' => $this->input->post('INV_USER_ROLE_ID'),
			'INV_USER_ENTITY_ID' => $this->input->post('INV_ROLE_NAME')
			);
		...
		//s print_r($data);	die;
		$result = $this->senddataurl('user',$data,'POST'); //kalau update ganti PUT jadi POST
		echo json_encode($result);
	}

	function masteruserhapus(){
		$data = ($_POST);
		$result = $this->senddataurl('user',$data,'DELETE');
		echo json_encode($result);
	}
	...
	function masteruserrolesearch(){
		$postdata = ($_POST);
		// print_r($postdata);die;
		$data = $this->senddataurl('userrole/search/',$postdata,'POST');
		// print_r($data);die;
		echo json_encode($data);
	}

	function masteruserrolesave(){
		$data = ($_POST);
		$result = $this->senddataurl('userrole',$data,'PUT');
		echo json_encode($result);

	}

	function masteruserroleedit(){
		$postdata = ($_POST);
		$data = $this->senddataurl('userrole/search/',$postdata,'POST');
		// print_r($data);	die;
		echo json_encode($data);
	...
	function masteruserroleupdate(){

		//cara pertama
		$data = ($_POST);
		$result = $this->senddataurl('userrole',$data,'POST'); //kalau update ganti PUT jadi POST
		echo json_encode($result);
	}

	function masteruserrolehapus(){
		$data = ($_POST);
		$result = $this->senddataurl('userrole',$data,'DELETE');
		echo json_encode($result);
	}

	/*umar master role*/

	function get_sub_entity(){
        $code=$this->input->post('INV_ENTITY_CODE');
        $data=$this->auth_model->get_subentity($code);
        echo json_encode($data);
	...
	function masterrole(){
		$role_id =  $this->session->userdata('role_id')	;
		$data['layanan'] = $this->auth_model->get_layanan($role_id);
		$data['entitys']	= $this->getdataurl('entity');
		$data['prods'] 	= $this->getdataurl('role');
		$data['unit1'] 	= $this->getdataurl('unit');
		$this->common_loader($data,'invoice/administrasi/masterrole/masterrole');
	}

	/*umar master role end */


	function masterrolesearch(){
		$postdata = ($_POST);

		$dataArray = $this->senddataurl('role/search/',$postdata,'POST');
		...
		foreach($dataArray as $key => $value) {
			...
			$data['data'][$key]['num'] = $num;
			$data['data'][$key]['action'] = '<td><button type="button"  onclick="editrole(\''.$data['data'][$key]['INV_ROLE_ID'].'\')"  value="" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#update_role"><i class="fa fa-pencil-square"></i></button></td>';
			$num++;
		}
		echo json_encode($data);
	}

	function masterrolesave(){

		//cara pertama
		$data = ($_POST);
		// print_r($data);	die;
		$result = $this->senddataurl('role',$data,'PUT');
		echo json_encode($result);

	}

	function masterroleedit(){
		$postdata = ($_POST);
		$id = $postdata['INV_ROLE_ID'];
		// print_r($id);die;
		$data = $this->getdataurl('role/'.$id);
		// print_r($data);die;
		echo json_encode($data);
	}

	function masterroleupdate(){

		//cara pertama
		$data = ($_POST);
		$result = $this->senddataurl('role',$data,'POST'); //kalau update ganti PUT jadi POST
		echo json_encode($result);
	}

	// function masternotaupdate(){
	// 	$postdata = ($_POST);
	// 	$code = array('INV_NOTA_CODE'=>$data['INV_NOTA_CODE']);
	// 	$result1 = $this->senddataurl('nota/search/',$code,'POST');
	// 	foreach ($result1 as $row) {
	// 		$data['INV_NOTA_ID']=$row->INV_NOTA_ID;
	// 	}
	// 	// print_r($data);die;
	// 	$data = $this->senddataurl('nota',$postdata,'POST');
	// 	echo json_encode($data);
	// }

	function masterrolehapus(){
		$data = ($_POST);
		$result = $this->senddataurl('role',$data,'DELETE');
		echo json_encode($result);
	}
...
public function print_card_delply($no_request,$port_code,$terminal_code,$hash=""){
	...
	function masterlognota(){
		$role_id =  $this->session->userdata('role_id')	;
		$data['layanan'] = $this->auth_model->get_layanan($role_id);
		$data['lognota'] = $this->getdataurl('lognota');

		$this->common_loader($data,'invoice/administrasi/masterlognota/masterlognota');
	}

	function masterlognotasearch(){
		$postdata = ($_POST);

		$data = $this->senddataurl('lognota/search/',$postdata,'POST');
		echo json_encode($data);
	...
	// 	$dataArray = $this->senddataurl('lognota/search/',$postdata,'POST');
