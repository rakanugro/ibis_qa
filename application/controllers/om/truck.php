...
//						$download_card
	...
	public function edit_tca($no_request,$message=null){
		...
		$kode_cabang_simkeu =  implode(', ', array_map(function ($result) {
					  return $result['KODE_CABANG_SIMKEU'];
					  					}, $result));					
		...
		else
		{
			...
			if($obj->data->request)
			{
				...
				$data['request_data'][0]['TERMINAL_NAME'] = $obj->data->request[0]->terminal_name;
				$data['request_data'][0]['ID_TERMINAL'] = $obj->data->request[0]->id_terminal;
		...
		$in_data="	<root>
					...
					<data>
						<tid>$tid</tid>
						<port_code>".$terminal_code."</port_code>
						<terminal_code>".$terminal_code."</terminal_code>
					</data>
				</root>";
	...
	public function create_register_id() {
		...
		$result1 = $this->user_model->get_idPCH($this->session->userdata('sub_group_phd'));
		
		$id_port =  implode(', ', array_map(function ($result1) {
					  return $result1['ID_PORT'];
					}, $result1));
		$id_company =  implode(', ', array_map(function ($result1) {
					  return $result1['ID_COMPANY'];
					}, $result1));
		$id_holding =  implode(', ', array_map(function ($result1) {
					  return $result1['ID_HOLDING'];
					}, $result1));
		...
		if($this->input->post()) {
			...
			$this->form_validation->set_rules($config); //setting rules inputan pemesanan pengeluaran
				...
				$in_data="<root>
					...
					<sc_code>123456</sc_code>
					<data>
						<terminal_code>$id_port</terminal_code>
						<customer_id>$customer_id</customer_id>
	...
	public function update_register_id() {
		...
		if($this->input->post()) {
			...
			$this->form_validation->set_rules($config); //setting rules inputan pemesanan pengeluaran
				...
				$in_data="<root>
					...
					<data>
						<port_code>".$port[0]."</port_code>
						<terminal_code>".$port[0]."</terminal_code>
						<customer_id>$customer_id</customer_id>
						<customer_name>$customer_name</customer_name>
						<address>$address</address>
						<truck_number>$truck_number</truck_number>
		...
		$customer_name=$this->session->userdata('customername_phd');
		$address=$this->session->userdata('address_phd');
		$terminal = $this->user_model->get_terminalListCargo($this->session->userdata('sub_group_phd'));

		
		if(count($terminal) > 0){
			$terminal_code = $terminal[0]['ID_PORT'];
		} else{			
			$terminal_code = '';
		}
