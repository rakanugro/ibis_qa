...
function submitrbm()
{
	...
	var operator = $( "#comboOperator option:selected" ).val();
...
/*	var martpd =arrtdd[2]+arrtdd[1]+arrtdd[0];*/
	...
	var url="<?=ROOT?>rbm/create_request_rbm";
	$.blockUI();
	$.post(url,{TERMINAL:terminal, ID_VSB_VOYAGE: id_vsb_voyage, VESSEL:vessel, VOYAGE_IN:voyage_in, VOYAGE_OUT:voyage_out, ETA:eta, ETD:etd, DELIVERY_DATE:delivery_date, NO_BL:no_bl, NO_DO:no_do, DO_DATE:do_date, SPPB_TYPE:sppbtype, NO_SPPB:no_sppb, DATE_SPPB:sppb_date, NO_SP_CUSTOM:no_sp_custom, DATE_SP_CUSTOM: sp_custom_date, FILE_SPPB:file_sppb, FILE_SP_CUSTOM:file_sp_custom, DATE_DISCHARGE:date_discharge, NO_BOOKING:no_book, DELIVERY_VIA:delivery_via,CUSTOMER_ID:customer_id,CUSTOMER_NAME:customer_name,NPWP:npwp,ADDRESS:address,OPERATOR:operator, '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'},
	...
	function(data) {
		...
		// console.log(obj);
		$("#submit_header").prop('disabled', false);
		if(obj.rc=="F") {
...
function cekTipeYd()
{
									...
									<div class="main-box-body clearfix">
										...
										<div class="form-group example-twitter-oss">
											...
											<div class="form-wrapper cf">
												...
												name="customer_autocomplete" placeholder="Search here..." title="Masukkan data kapal" required>
											...
											<div class="form-group example-twitter-oss">
												<label for="exampleAutocomplete">Operator</label>
							                      <div id="comboOperator"></div>
											</div>
										</div>
  ...
  function search_vessel(){
      ...
      if(vesselname == ''){
        ...
        $.get("<?=ROOT?>rbm/auto_vessel_delivery?",{term : vesselname, port: port}, function(data){
  ...
  function complete($vessel,$voyin,$voyout,$voyage,$id_vsb_voyage,$vessel_code,$call_sign,$date_discharge,$etd,$eta,$nobook){
      ...
      $.post("<?=ROOT?>rbm/auto_operator",{  '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
                                                                    id_vsb_voyage: $id_vsb_voyage,
                                                                    port: $('#port').val()
                                                                  },function(data){
          $("#comboOperator").html(data);
      });
  }
