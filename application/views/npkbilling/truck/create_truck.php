			...
			<div class="main-box">
				...
				<div class="main-box-body clearfix">
					<div class="form-group col-xs-12">
						...
						<input name="TRUCK_ID" id="TRUCK_ID" type="text" class="form-control" placeholder="TID Truck" readonly>
					</div>
					<div class="form-group col-xs-12">
						<label>Vechile Type</label>
						<span style="color: red">*</span>
						<select id="TRUCK_TYPE" name="TRUCK_TYPE" class="form-control" onchange="getnameoperation(this);">
							<option value="not-selected"> -- Please Vechile Type  -- </option>
							<?php foreach($kendaraan as $ken){ ?>
								<option value="<?=$ken->type_id?>"><?=$ken->type_name?></option>
							<?php } ?>
						</select>
						<input type="hidden" class="form-control" id="TRUCK_TYPE_NAME" name="TRUCK_TYPE_NAME" readonly>
					</div>
					<div class="form-group col-xs-12">
						<label for="exampleTooltip">Truck Company Name</label>
						<span style="color: red">*</span>
						<input name="TRUCK_CUST_NAME" id="TRUCK_CUST_NAME" type="text" class="form-control" placeholder="autocomplete" >
						<input type="hidden" class="form-control" id="TRUCK_CUST_ID" name="TRUCK_CUST_ID">
					</div>
					<div class="form-group col-xs-6">
						<label for="exampleTooltip">Plat Nomor</label>
						<span style="color: red">*</span>
						<input name="TRUCK_PLAT_NO" id="TRUCK_PLAT_NO" type="text" class="form-control" placeholder="Plat Nomor">
					</div>
					<div class="form-group col-xs-6">
						...
						<div class="input-group">
							...
							<input id="TRUCK_PLAT_EXP" name="TRUCK_PLAT_EXP" type="text" class="form-control" id="datepickerDate" placeholder="Expired Date">
						</div>
					</div>
					<div class="form-group col-xs-12">
						<label for="exampleTooltip">RFID Code</label>
						<input name="TRUCK_RFID" id="TRUCK_RFID" type="text" class="form-control" placeholder="RFID Code">
					</div>
				</div>
			</div>
		</div>
			...
			<div class="main-box clearfix">
				...
				<div class="main-box-body clearfix">		
					<div class="form-group example-twitter-oss pull-right">
						<button type="button" class="btn btn-danger btn-footer " id="save_truck" data-toggle="modal" data-target="#modal-default"><span class="glyphicon glyphicon-ok-sign"></span>&nbsp;&nbsp;Save</button>
...
<script>
	...
	$(document).ready(function() {
		...
		$('#save_truck').click(function(){
			...
			var TRUCK_PLAT_NO		= $('#TRUCK_PLAT_NO').val();
			...
			if (TRUCK_TYPE == 'not-selected') {
				...
				return false;
			} else if (TRUCK_PLAT_NO == '') {
				alert("Please Plat Nomor !");
				$('#TRUCK_PLAT_NO').focus()
				return false;
		...
		$("#TRUCK_PLAT_NO").keyup(function(){
			var tgl     	= $('#TRUCK_PLAT_NO').val();
			var str 		= $("#TRUCK_PLAT_NO").val();
			var tid_truck 	= str.toUpperCase();
    		$("#TRUCK_ID").val(tid_truck.replace(/ /g, ""));
	...
	$('#btn-modal-kirim').click(function(){ saveTruck(); return false; });
	...
	function saveTruck() {
		...
		var url = "<?=ROOT?>npkbilling/truckregistrasi/createRequstTruck";
		$.blockUI();
