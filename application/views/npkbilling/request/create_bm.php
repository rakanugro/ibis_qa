			...
			<div class="main-box">
				...
				<div class="main-box-body clearfix">
					<div class="form-group col-xs-6">
						...
						<input type="text" id="BM_PIB_PEB_NO" name="BM_PIB_PEB_NO" class="form-control" placeholder="No PEB/PIB" required>
					</div>
					<div class="form-group col-xs-6">
						<label for="exampleAutocomplete">Tanggal PEB/PIB</label>
                      	<div class="input-group">
							<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
							<input id="BM_PIB_PEB_DATE" name="BM_PIB_PEB_DATE" type="text" placeholder="Tanggal PEB/PIB" class="form-control" id="datepickerDate">
					...
					<div class="form-group col-xs-12">
						...
						<input id="BM_NPE_SPPB_NO" name="BM_NPE_SPPB_NO" type="text" placeholder="No NPE/SPPB" class="form-control" id="exampleTooltip">
					</div>
				</div>
			</div>
		</div>
...
<script>
	...
	$(document).ready(function() {
		...
		$("#add_file").on("click", function () {
			...
			var BM_PBM_ID				= $('#BM_PBM_ID');
			var BM_SHIPPING_AGENT_ID	= $('#BM_SHIPPING_AGENT_ID');
			var BM_TRADE_TYPE			= $('#BM_TRADE_TYPE');
			var BM_VESSEL				= $('#BM_VESSEL');
			var BM_TERMINAL_CODE		= $('#BM_TERMINAL_CODE');

			$('#DOC_NO'+counterdoc).keypress(function(){
				if (BM_TERMINAL_CODE.val() != 'not-selected' && BM_PBM_ID.val() != 'not-selected' && BM_SHIPPING_AGENT_ID.val() != 'not-selected' && BM_TRADE_TYPE.val() != 'not-selected' && BM_VESSEL.val() != '')  {
		...
		BM_TERMINAL_CODE.change(function(){
			if (BM_PBM_ID.val() != 'not-selected' && BM_SHIPPING_AGENT_ID.val() != 'not-selected' && BM_TRADE_TYPE.val() != 'not-selected' && BM_VESSEL.val() != '')  {
			...
			if(BM_TERMINAL_CODE.val() == 'not-selected'){
				$("#btn-show").prop('disabled', true);
			}
		})

		BM_PBM_ID.change(function(){
			if (BM_TERMINAL_CODE.val() != 'not-selected' && BM_SHIPPING_AGENT_ID.val() != 'not-selected' && BM_TRADE_TYPE.val() != 'not-selected' && BM_VESSEL.val() != '')  {
				if(typeof($('#DOC_NAME'+counterdoc)) != "undefined"){
					if($('#DOC_NAME'+counterdoc).val() != '' && $('#DOC_NO'+counterdoc).val() != ''){
						$("#btn-show").prop('disabled', false);
					}
			...
			if(BM_PBM_ID.val() == 'not-selected'){
				$("#btn-show").prop('disabled', true);
			}
		});

		BM_SHIPPING_AGENT_ID.change(function(){
			if (BM_TERMINAL_CODE.val() != 'not-selected' && BM_PBM_ID.val() != 'not-selected' && BM_TRADE_TYPE.val() != 'not-selected' && BM_VESSEL.val() != '')  {
				if(typeof($('#DOC_NAME'+counterdoc)) != "undefined"){
					if($('#DOC_NAME'+counterdoc).val() != '' && $('#DOC_NO'+counterdoc).val() != ''){
						$("#btn-show").prop('disabled', false);
					}
			...
			if(BM_SHIPPING_AGENT_ID.val() == 'not-selected'){
				$("#btn-show").prop('disabled', true);
			}
		});

		BM_TRADE_TYPE.change(function(){
			if (BM_TERMINAL_CODE.val() != 'not-selected' && BM_PBM_ID.val() != 'not-selected' && BM_SHIPPING_AGENT_ID.val() != 'not-selected' && BM_VESSEL.val() != '')  {
				if(typeof($('#DOC_NAME'+counterdoc)) != "undefined"){
					if($('#DOC_NAME'+counterdoc).val() != '' && $('#DOC_NO'+counterdoc).val() != ''){
						$("#btn-show").prop('disabled', false);
					}
			...
			if(BM_TRADE_TYPE.val() == 'not-selected'){
				$("#btn-show").prop('disabled', true);
			}
		});
		...
		$.ajax({
			...
			success: function(data){
				...
				$('#BM_TERMINAL_CODE').append(toAppend);
			}
		});
		...
		$('#BM_VESSEL_NAME').autocomplete({
			...
			select: function (event, ui) {
				...
				if (BM_TERMINAL_CODE.val() != 'not-selected' && BM_PBM_ID.val() != 'not-selected' && BM_SHIPPING_AGENT_ID.val() != 'not-selected' && BM_TRADE_TYPE.val() != 'not-selected')  {
		...
		arrData = {
            ...
            "HEADER": {
                ...
                "VALUE"  : [{
					...
					"BM_NO": no_req,
					"BM_STATUS": 1,
					"BM_BRANCH_ID":  $('#BM_TERMINAL_CODE').find('option:selected').attr('brchid'),
					"BM_BRANCH_CODE":  $('#BM_TERMINAL_CODE').find('option:selected').attr('brchcode'),
					"BM_CREATE_BY": 1,
					...
					"BM_CUST_NAME": "<?=$this->session->userdata('customernamealt_phd')?>",
