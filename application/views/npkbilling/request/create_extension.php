			...
			<div class="main-box">
				...
				<div class="main-box-body clearfix">
					...
					<div class="form-group col-xs-12">
						...
						<input id="EQ_UNIT_QTY" name="EQ_UNIT_QTY" type="number" min="0" class="form-control" placeholder="Jumlah/Durasi Pemakaian">
					</div>
					<div class="form-group col-xs-12">
						<label for="exampleTooltip">Jumlah Alat</label>
						<input id="EQ_QTY_RENT" name="EQ_QTY_RENT" type="number" min="0" class="form-control" placeholder="Jumlah Alat">
					</div>
					<div class="form-group col-xs-12">
						<label>Kemasan</label>
...
<script>
	...
	$(document).ready(function() {
		...
		$('#DEL_EXT_FROM').autocomplete({
			source: function( request, response ) {
				$.ajax({
					url: "<?=ROOT?>npkbilling/request_extension/get_nomor_request",
					type: 'GET',
					dataType: "json",
					data: {
						search: request.term
			...
			select: function (event, ui) {
				...
				$.ajax({
					url: "<?=ROOT?>npkbilling/request_extension/auto_generate_by_noreq",
					type: 'GET',
					dataType: "json",
					data: {
					...
					success: function( data ) {
						...
						data.FILE.forEach(function(file, index){
							if (data.FILE.length != 0) {
								...
								cols += '<div class="col-xs-5"><label>Upload Dokumen</label><input type="file" accept=".pdf" name="DOC_NAME'+counterdoc+'" id="DOC_NAME'+counterdoc+'" doc_name="'+file.doc_name+'" data-toggle="tooltip" data-placement="bottom" size="100" onchange="encodedoc('+counterdoc+')"><a href="<?=apiUrl?>/'+file.doc_path+'" target="_blank">'+file.doc_name+'</a></div>';
								...
								cols +=	'<input type="hidden" id="DOC_BASH'+counterdoc+'" name="DOC_BASH'+counterdoc+'" class="form-control" data-toggle="tooltip" data-placement="bottom" maxlength="40" value="'+file.base64+'">';
	...
	function save_extension() {
		...
		$('#detail-list tbody tr').each(function(){
			...
			if ($(this).find('.TBL_DTL_CUST_ID').html() != '') {
				console.log($(this).find('.TBL_DTL_CUST_ID').html());
			}
			details.push(temp);
		});
	...
	function extension_log(no_req){
		$.ajax({
			url: "<?=ROOT?>npkbilling/transaction_log/extension_insert_log",
			type: 'POST',
			//dataType: 'json',
			data: {
				'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
