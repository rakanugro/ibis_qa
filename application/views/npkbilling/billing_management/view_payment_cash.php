                ...
                <div class="main-box-body clearfix">
                    <?php foreach($bank as $banks) { ?>
                        <div class="col-md-4 col-lg-4 col-sm-4">
                            <label>
                            <input type="radio" name="PAY_BANK_CODE" onclick="radioBankChange(this);" value="<?php echo $banks->bank_code.'||'.$banks->bank_name.'||'.$banks->branch_id.'||'.$banks->account_no.'||'.$banks->account_name.'||'.$banks->bank_id ?>" class="card-input-element" />
                                   ...
                                   <!--  <?php 
                                    ...
                                    if($isPFs > 0) { ?>
                                        <tr>
                                            ...
                                            <td><p class="text-right">Rp. <?php echo number_format($amount,2,",",".") ?></p></td>
                                        </tr>
                                        <?php $no++;  }
                                    ...
                                    <tr>
                                        ...
                                        <td><strong><p class="text-right">Rp. <?php echo number_format($total_amount,2,",",".") ?></p></strong></td>
                    ...
                    <div class="main-box-body clearfix">
                       ...
                       <!--  <div class="form-group col-xs-6">
                        ...
                        <div class="form-group col-xs-6">
                            ...
                            <input name="pay_amount" id="pay_amount" type="text" class="form-control" placeholder="payment_amount" value ="<?php echo $total_amount ?>">
                        </div>
                        <div class="form-group col-xs-6">
                            <label for="exampleTooltip">Payment Date</label>
                            <input name="pay_date" id="pay_date" type="text" class="form-control" placeholder="payment_date">
                        </div>
                       <!--  <div class="form-group col-xs-6">
                        ...
                        <div class="form-group example-twitter-oss">
                            <input type="button" onclick="save_payment()" value="Confirm" id="confirm" name="confirm" class="btn btn-danger"/>
...
{   
    ...
    var account_no = split_bank_radio[3];
    var account_name = split_bank_radio[4];
    var bank_id = split_bank_radio[5];
    ...
    arrData = {
		...
		"pay_branch_id": "<?=$terminal[0]['BRANCH_ID']; ?>" ,
        "pay_branch_code" : "<?=$terminal[0]['BRANCH_CODE']; ?>" ,
		"pay_account_no": account_no,
		"pay_account_name": account_name,
		"pay_sender_bank_code": $('#pay_sender_bank_code').val(),
		"pay_sender_bank_name": $('#pay_sender_bank_name').val(),
		"pay_sender_account_no": $('#pay_sender_account_no').val(),
		"pay_sender_account_name": $('#pay_sender_account_name').val(),
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.blockUI();
            $.ajax({
                url: "<?=ROOT?>npkbilling/payment_cash/save_payment_cash",
                type: 'POST',
                dataType: 'json',
                data: {
                ...
                success: function(data) {
                    var returnan = JSON.parse(data.data);
                       if (data.success === 'S') {
                        $.unblockUI();
                        Swal.fire({
                            icon: 'success',
                            title: returnan['result'],
                            showConfirmButton: false,
                            timer: 1500
                        });
                        setTimeout(function(){ window.location = "<?=ROOT?>npkbilling/payment_cash"; }, 3000);	
                    } else {
