	...
	<div class="col-lg-12 ">
		<div class="main-box">
			...
			<div class="main-box-body clearfix">
				...
				<div class="form-group col-xs-4">
					<label for="datepickerDate">Tanggal strippeiving</label>
...
<script>
	...
	$('#request_id_reff').autocomplete({
			source: function( request, response ) {
				$.ajax({
					url: "<?=ROOT?>npksbilling/strippingbatal/get_nomor_request",
					type: 'GET',
					dataType: "json",
					data: {
						search: request.term
			...
			select: function (event, ui) {
				...
				$.ajax({
					url: "<?=ROOT?>npksbilling/strippingbatal/auto_generate_by_noreq",
					type: 'GET',
					dataType: "json",
					data: {
					...
					success: function( data ) {
						...
						var req_method = (data['HEADER'][0]['stripp_paymethod'] ==2)? "PIUTANG" : "ADVANCE PAYMENT";
						var req_to = '';
						...
						$('#pbm').val(data['HEADER'][0]['stripp_pbm_name']);
						
						$('#from').val(req_to);
						$('#penumpukan_oleh').val(data['HEADER'][0]['stripp_stackby_name']);
						$('#payment_method').val(req_method);

						//customer
						$('#stripp_cust_name').val(data['HEADER'][0]['stripp_cust_name']);
						$('#stripp_cust_npwp').val(data['HEADER'][0]['stripp_cust_npwp']);
						$('#stripp_cust_address').val(data['HEADER'][0]['stripp_cust_address']);
						

											
						// VESSEL
						$('#vessel').val(data['HEADER'][0]['stripp_vessel_name']);
						$('#nama_agen').val(data['HEADER'][0]['stripp_vessel_agent_name']);
						$('#no_pkk').val(data['HEADER'][0]['stripp_vessel_pkk']);
						$('#voyage_in').val(data['HEADER'][0]['stripp_voyin']);
						$('#voyage_out').val(data['HEADER'][0]['stripp_voyout']);
						$('#port_of_loading').val(data['HEADER'][0]['stripp_vessel_pol']);
						$('#port_of_destination').val(data['HEADER'][0]['stripp_vessel_pod']);
