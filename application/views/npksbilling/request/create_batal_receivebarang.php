		...
		<div class="main-box">
			...
			<div class="main-box-body clearfix">
				...
				<div class="form-group col-xs-6">
					...
					<input name="rec_cargo_branch_id" id="rec_cargo_branch_id" type="hidden" class="form-control" placeholder="rec_cargo_branch_id" required="" readonly="">
					<input name="rec_cargo_branch_code" id="rec_cargo_branch_code" type="hidden" class="form-control" placeholder="rec_cargo_branch_code" required="" readonly="">
				</div>
			</div>
		</div>
	</div>
		...
		<div class="main-box">
			...
			<div class="main-box-body clearfix">
				<div class="form-group col-xs-12">
					...
					<input name="rec_cust_name" id="rec_cust_name" type="text" class="form-control" placeholder="Autocomplete" required="" required="">
				</div>
				<div class="form-group col-xs-12">
					<label>NPWP</label>
					<input name="rec_cust_npwp" id="rec_cust_npwp" type="text" class="form-control" placeholder="Auto Generate" required="" readonly="">
				</div>
				<div class="form-group col-xs-12">
					<label>Alamat</label>
					<input name="rec_cust_address" id="rec_cust_address" type="text" class="form-control" placeholder="Autocomplete" required="" readonly="">
				</div>
...
<script>
		...
		arrData = 
			{
				...
				"service_branch_id" : $('#rec_cargo_branch_id').val(),
				"service_branch_code" : $('#rec_cargo_branch_code').val(),
				"data": [
					"HEADER",
					"DETAIL"
				...
				"HEADER": {
					...
					"VALUE": [{
						...
						"CANCELLED_STATUS": 1,
						"CANCELLED_TYPE" : 10,
						"CANCELLED_BRANCH_ID" : $('#rec_cargo_branch_id').val(), 
						"CANCELLED_BRANCH_CODE" : $('#rec_cargo_branch_code').val(),
						"APP_ID" : 2 
	...
	$('#request_id_reff').autocomplete({
			source: function( request, response ) {
				$.ajax({
					url: "<?=ROOT?>npksbilling/receivebarangbatal/get_nomor_request",
					type: 'GET',
					dataType: "json",
					data: {
						search: request.term
			...
			select: function (event, ui) {
				...
				$.ajax({
					url: "<?=ROOT?>npksbilling/receivebarangbatal/auto_generate_by_noreq",
					type: 'GET',
					dataType: "json",
					data: {
					...
					success: function( data ) {
						...
						var req_method = (data['HEADER'][0]['rec_cargo_paymethod'] == 2)? "PIUTANG" : "ADVANCED PAYMENT";
						var req_to = '';
						if ((data['HEADER'][0]['rec_cargo_from'] == 1)){
							req_to = "DEPO";
						}else if((data['HEADER'][0]['rec_cargo_from'] == 2)){
							req_to = "TPK";
						}else{
							req_to = "TONGKANG";
						}
						...
						$('#rec_cargo_branch_id').val(data['HEADER'][0]['rec_cargo_branch_id']);
						$('#rec_cargo_branch_code').val(data['HEADER'][0]['rec_cargo_branch_code']);
						...
						//customer
						$('#rec_cust_name').val(data['HEADER'][0]['rec_cargo_cust_name']);
						$('#rec_cust_npwp').val(data['HEADER'][0]['rec_cargo_cust_npwp']);
						$('#rec_cust_address').val(data['HEADER'][0]['rec_cargo_cust_address']);
						...
						$('#port_of_loading').val(data['HEADER'][0]['rec_cargo_pol']);
						$('#port_of_destination').val(data['HEADER'][0]['rec_cargo_pod']);
						  ...
						  var no = 1;
							...
							$('#detail-list tbody').append(
								'<tr>' +
									...
									//'<td>'+ abc.rec_cargo_dtl_owner_name +'</td>' +
									
									'<td>' +
										'<input type="text" name="name1" id="rec_cargotxt'+i+'"/>' +
										'<input type="hidden" name="name1" id="rec_cargo_dtl_id'+i+'" value="'+abc.rec_cargo_dtl_id +'"/>' +
										'<input type="text" name="name1" id="si_no'+i+'" value="'+abc.rec_cargo_dtl_si_no +'"/>' +
