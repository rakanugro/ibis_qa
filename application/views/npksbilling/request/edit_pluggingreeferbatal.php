...
<script>
	      	...
	      	//for (var i = 0, row; row = table.rows[i]; i++) {
	            ...
	            $("input:checkbox[name=check]:checked").each(function () {
	              // console.log('ad');
			 		...
			 		var temp = {
			 			"CANCL_DTL_ID":$('#plug_dtl_id' + $(this).val() ).val(),
		...
		$.ajax({
			...
			success: function( data ) {
				...
				if (data.success === 'S') {
					...
					var cancelled_req_no = response.header.plug_no;
	...
	$.ajax({
      ...
      url: "<?=ROOT?>npksbilling/pluggingreeferbatal/preview_edit/"+id_param,
      type: "GET",
      dataType : 'json',
      success: function (data) {
      	//console.log(data);
        ...
        console.log(data);
						...
						if ((data['HEADER'][0]['plug_from'] == 1)){
       					...
       					$('#plug_branch_id').val(data['HEADER_REQ'][0]['plug_branch_id']);
						$('#plug_branch_code').val(data['HEADER_REQ'][0]['plug_branch_code']);

       					$('#cancelled_id').val(data["HEADER"][0]['cancelled_id']);
       					$('#request_id_reff').val(data["HEADER_REQ"][0]['plug_no']);
		        	...
		        	//console.log(row);
						...
						// DETAILS
						  ...
						  var no = 1;
							...
							//if(abc.plug_dtl_iscancelled === "Y" ) document.getElementById("chk").checked == true;
