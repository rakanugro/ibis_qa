		...
		<div class="main-box">
			...
			<div class="main-box-body clearfix">
				...
				<div class="form-group col-xs-6">
					...
					<input name="penumpukan_oleh" id="penumpukan_oleh" type="text" class="form-control" placeholder="" required="" readonly="">
				</div>
				<div class="form-group col-xs-6">
				...
				<div class="form-group col-xs-6">
					...
					<input name="cancelled_branch_id" id="cancelled_branch_id" type="hidden" class="form-control" placeholder="cancelled_branch_id" required="" readonly="">
					<input name="cancelled_branch_code" id="cancelled_branch_code" type="hidden" class="form-control" placeholder="cancelled_branch_code" required="" readonly="">
		...
		<div class="main-box">
			...
			<div class="main-box-body clearfix">
				<div class="form-group col-xs-12">
					...
					<input name="rec_cargo_cust_name" id="rec_cargo_cust_name" type="text" class="form-control" placeholder="Autocomplete" required="" required="">
				</div>
				<div class="form-group col-xs-12">
					<label>NPWP</label>
					<input name="rec_cargo_cust_npwp" id="rec_cargo_cust_npwp" type="text" class="form-control" placeholder="Auto Generate" required="" readonly="">
				</div>
				<div class="form-group col-xs-12">
					<label>Alamat</label>
					<input name="rec_cargo_cust_address" id="rec_cargo_cust_address" type="text" class="form-control" placeholder="Autocomplete" required="" readonly="">
				</div>
...
<script>
	...
	function save_request() {
		...
		$('#modal-save').modal('hide');
	        ...
	        for ( var i = 0; i < checkBoxes.length; i++) {
	            if ($('#rec_cargotxt'+i).val() >0) {  
	            	
	                message += "   " + $('#si_no' + i).val();
	                message += "   " + $('#rec_cargotxt' + i).val();
	                message += " -> "+no;
	                message += "\n";
					var temp = {
						"CANCL_HDR_ID": $('#cancelled_id' + i).val(),
			 			"CANCL_DTL_ID": $('#rec_cargo_dtl_id' + i).val(),
			 		...
			 		arrDataEdit = {
                            ...
                            "update" : 
                            {
                                "REC_CARGO_DTL_ISCANCELLED" : "Y"
                            },
                            "where" : 
                            {
                                "REC_CARGO_DTL_ID" : $('#rec_cargo_dtl_id' + i).val()
                            }
                        };

        $.ajax({
			url: "<?=ROOT?>npksbilling/receivebarangbatal/update_request",
			type: 'POST',
			dataType: 'json',
			data: { 
		...
		arrData = 
			{
				...
				"service_branch_id" : $('#cancelled_branch_id').val(),
				"service_branch_code" : $('#cancelled_branch_code').val(),
				"data": [
					"HEADER",
					"DETAIL"
				...
				"HEADER": {
					...
					"VALUE": [{
						...
						"CANCELLED_STATUS": 1,
						"CANCELLED_TYPE" : 10,
						"CANCELLED_BRANCH_ID" : $('#cancelled_branch_id').val(),
						"CANCELLED_BRANCH_CODE" : $('#cancelled_branch_code').val(),
						"APP_ID" : 2 
		...
		$.ajax({
			url: "<?=ROOT?>npksbilling/receivebarangbatal/save_request",
			type: 'POST',
			dataType: 'json',
			data: { 
				'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
			...
			success: function( data ) {
				...
				if (data.success === 'S') {
					...
					var cancelled_req_no = response.header.rec_cargo_no;
					...
					//extension_log(rec_no);

					setTimeout(function(){ window.location = "<?=ROOT?>npksbilling/receivebarangbatal/"; }, 2000);	
				} else {
					$.unblockUI();
					alert('Data Gagal Disimpan;');
				}
	...
	$.ajax({
      ...
      url: "<?=ROOT?>npksbilling/receivebarangbatal/preview_edit/"+id_param,
      type: "GET",
      dataType : 'json',
      success: function (data) {
      	//console.log(data);
        ...
        console.log(data);
        				...
        				var req_method = (data['HEADER'][0]['rec_paymethod'] != 2)? "PIUTANG" : "ADVANCED PAYMENT";
						var req_to = '';
        				...
        				$('#cancelled_branch_id').val(data["HEADER"][0]['cancelled_branch_id']);
        				$('#cancelled_branch_code').val(data["HEADER"][0]['cancelled_branch_code']);
        				$('#cancelled_id').val(data["HEADER"][0]['cancelled_id']);
       					$('#nomor_request').val(data["HEADER"][0]['cancelled_no']);
       					
       					$('#request_id_reff').val(data["HEADER"][0]['rec_cargo_no']);
						$('#pbm').val(data['HEADER'][0]['rec_cargo_pbm_name']);
						
						$('#from').val(req_to);
						$('#penumpukan_oleh').val(data['HEADER'][0]['rec_cargo_stackby_name']);
						$('#payment_method').val(req_method);

						//customer
						$('#rec_cargo_cust_name').val(data['HEADER'][0]['rec_cargo_cust_name']);
						$('#rec_cargo_cust_npwp').val(data['HEADER'][0]['rec_cargo_cust_npwp']);
						$('#rec_cargo_cust_address').val(data['HEADER'][0]['rec_cargo_cust_address']);
						

											
						// VESSEL
						$('#vessel').val(data['HEADER'][0]['rec_cargo_vessel_name']);
						$('#nama_agen').val(data['HEADER'][0]['rec_cargo_vessel_agent_name']);
						$('#no_pkk').val(data['HEADER'][0]['rec_cargo_vessel_pkk']);
						$('#voyage_in').val(data['HEADER'][0]['rec_cargo_voyin']);
						$('#voyage_out').val(data['HEADER'][0]['rec_cargo_voyout']);
						$('#port_of_loading').val(data['HEADER'][0]['rec_cargo_vessel_pol']);
						$('#port_of_destination').val(data['HEADER'][0]['rec_cargo_vessel_pod']);
		        	...
		        	//console.log(row);
						...
						// DETAILS
						  ...
						  var no = 1;
							...
							//if(abc.rec_cargo_dtl_iscancelled === "Y" ) document.getElementById("chk").checked == true;

							$('#detail-list tbody').append(
								'<tr>' +
									'<td>'+ no++ +'</td>' +
									'<td>'+ abc.rec_cargo_dtl_si_no +'</td>' +
									'<td>'+ abc.rec_cargo_dtl_pkg_name+'</td>' +
									'<td>'+ abc.rec_cargo_dtl_unit_name+'</td>' +
									'<td>'+ abc.rec_cargo_dtl_qty+'</td>' +
									'<td>'+ abc.rec_cargo_dtl_character_name+'</td>' +
									'<td>'+ abc.rec_cargo_dtl_cmdty_name +'</td>' +
									'<td>'+ abc.rec_cargo_dtl_via+'</td>' +
									'<td>'+ abc.rec_cargo_dtl_rec_date+'</td>' +
									
									'<td>' +
										'<input type="text" name="name1" id="rec_cargotxt'+i+'" value="'+abc.cancl_qty+'"/>' +
										'<input type="hidden" name="name1" id="rec_cargo_dtl_id'+i+'" value="'+abc.rec_cargo_dtl_id +'"/>' +
										'<input type="hidden" name="name1" id="si_no'+i+'" value="'+abc.rec_cargo_dtl_si_no +'"/>' +
										'<input type="hidden" name="name1" id="unit_id'+i+'" value="'+abc.rec_cargo_dtl_unit_id +'"/>' +
										'<input type="hidden" name="name1" id="pkg_id'+i+'" value="'+abc.rec_cargo_dtl_pkg_id +'"/>' +
										'<input type="hidden" name="name1" id="pkg_parent_id'+i+'" value="'+abc.rec_cargo_dtl_pkg_parent_id +'"/>' +
										'<input type="hidden" name="name1" id="cmdty_id'+i+'" value="'+abc.rec_cargo_dtl_cmdty_id +'"/>' +
									'</td>' +
								'</tr>'
							);	
							i++;
