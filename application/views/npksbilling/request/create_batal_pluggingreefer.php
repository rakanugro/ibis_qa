	...
	<div class="col-lg-12">
		<div class="main-box">
			...
			<div class="main-box-body clearfix">
				...
				<div class="form-group col-xs-4">
					<label>Penumpukan Dari</label>
					<input name="penumpukan_dari" id="penumpukan_dari" type="text" class="form-control" placeholder="Autocomplete" required="" readonly="">
				</div>
				<div class="form-group col-xs-4">
					...
					<input name="plug_branch_id" id="plug_branch_id" type="hidden" class="form-control" placeholder="plug_branch_id" required="" readonly="">
					<input name="plug_branch_code" id="plug_branch_code" type="hidden" class="form-control" placeholder="plug_branch_code" required="" readonly="">
				</div>
			</div>
		</div>
	</div>
			...
			<div class="main-box-body clearfix">
				...
				<div class="form-group col-xs-4">
					...
					<div class="input-group">
						...
						<input id="DTL_TGL_KEGIATAN" name="DTL_TGL_KEGIATAN" type="text" class="form-control" value="<?=date('m/d/Y')?>" required="" readOnly>
					</div>
				</div>
				<div class="form-group col-xs-6">
					<label>Type</label>
...
<script>
	            ...
	            $("input:checkbox[name=check]:checked").each(function () {
	              // console.log('ad');
	         		...
	         		arrDataEdit = {
                            ...
                            "update" : 
                            {
                                "PLUG_DTL_ISCANCELLED" : "Y"
                            },
                            "where" : 
                            {
                                "PLUG_DTL_ID" : $('#plug_dtl_id' + $(this).val()).val()
                            }
                        };

        $.ajax({
			url: "<?=ROOT?>npksbilling/pluggingreeferbatal/update_request",
			type: 'POST',
			dataType: 'json',
			data: { 
	...
	//}
		...
		arrData = 
			{
				...
				"service_branch_id" : $('#plug_branch_id').val(),
				"service_branch_code" : $('#plug_branch_code').val() ,
				"data": [
					"HEADER",
					"DETAIL"
				...
				"HEADER": {
					...
					"VALUE": [{
						...
						"CANCELLED_STATUS": 1,
						"CANCELLED_TYPE" : 8,
						"CANCELLED_BRANCH_ID" : $('#plug_branch_id').val() ,
						"CANCELLED_BRANCH_CODE" : $('#plug_branch_code').val(), 
						"APP_ID" : 2 
					}]
				},
		...
		$.ajax({
			url: "<?=ROOT?>npksbilling/pluggingreeferbatal/save_request",
			type: 'POST',
			dataType: 'json',
			data: { 
				'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
			...
			success: function( data ) {
				...
				if (data.success === 'S') {
					...
					setTimeout(function(){ window.location = "<?=ROOT?>npksbilling/pluggingreeferbatal/"; }, 2000);	
				} else {
					$.unblockUI();
					alert('Data Gagal Disimpan;');
				}
	...
	$('#request_id_reff').autocomplete({
			source: function( request, response ) {
				$.ajax({
					url: "<?=ROOT?>npksbilling/pluggingreeferbatal/get_nomor_request",
					type: 'GET',
					dataType: "json",
					data: {
						search: request.term
			...
			select: function (event, ui) {
				...
				$.ajax({
					url: "<?=ROOT?>npksbilling/pluggingreeferbatal/auto_generate_by_noreq",
					type: 'GET',
					dataType: "json",
					data: {
					...
					success: function( data ) {
						...
						var req_method = (data['HEADER'][0]['plug_paymethod'] == 2)? "PIUTANG" : "ADVANCED PAYMENT";
						var req_to = '';
						...
						$('#plug_branch_id').val(data['HEADER'][0]['plug_branch_id']);
						$('#plug_branch_code').val(data['HEADER'][0]['plug_branch_code']);

						$('#pbm').val(data['HEADER'][0]['plug_pbm_name']);
						
						$('#from').val(req_to);
						$('#penumpukan_oleh').val(data['HEADER'][0]['plug_bplug_from']);
						$('#payment_method').val(req_method);
						$('#receiving_from').val(data['HEADER'][0]['plug_paymethod']);

						//customer
						$('#plug_cust_name').val(data['HEADER'][0]['plug_cust_name']);
						$('#plug_cust_npwp').val(data['HEADER'][0]['plug_cust_npwp']);
						$('#plug_cust_address').val(data['HEADER'][0]['plug_cust_address']);
						

											
						// VESSEL
						$('#vessel').val(data['HEADER'][0]['plug_vessel_name']);
						$('#nama_agen').val(data['HEADER'][0]['plug_vessel_agent_name']);
						$('#no_pkk').val(data['HEADER'][0]['plug_vessel_pkk']);
						$('#voyage_in').val(data['HEADER'][0]['plug_voyin']);
						$('#voyage_out').val(data['HEADER'][0]['plug_voyout']);
						$('#port_of_loading').val(data['HEADER'][0]['plug_pol']);
						$('#port_of_destination').val(data['HEADER'][0]['plug_pod']);
							...
							$('#detail-list tbody').append(
								'<tr>' +
									...
									'<td>' +
										...
										'<input type="hidden" name="name1" id="plug_dtl_id'+i+'" value="'+abc.plug_dtl_id +'"/>' +
										'<input type="hidden" name="name2" id="dtl_cont'+i+'" value="'+abc.plug_dtl_cont +'"/>' +
