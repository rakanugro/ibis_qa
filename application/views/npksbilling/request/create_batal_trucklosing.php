...
<script>
	...
	$('#request_id_reff').autocomplete({
			source: function( request, response ) {
				$.ajax({
					url: "<?=ROOT?>npksbilling/trucklosingbatal/get_nomor_request",
					type: 'GET',
					dataType: "json",
					data: {
						search: request.term
			...
			select: function (event, ui) {
				...
				$.ajax({
					url: "<?=ROOT?>npksbilling/trucklosingbatal/auto_generate_by_noreq",
					type: 'GET',
					dataType: "json",
					data: {
					...
					success: function( data ) {
						...
						$('#tl_branch_id').val(data['HEADER'][0]['tl_branch_id']);
						$('#tl_branch_code').val(data['HEADER'][0]['tl_branch_code']);

						$('#pbm').val(data['HEADER'][0]['tl_pbm_name']);
						
						$('#receiving_from').val(req_to);
						$('#penumpukan_oleh').val(data['HEADER'][0]['tl_stackby_name']);
						$('#payment_method').val(req_method);
						//$('#receiving_from').val(data['HEADER'][0]['tl_paymethod']);

						//customer
						$('#tl_cust_name').val(data['HEADER'][0]['tl_cust_name']);
						$('#tl_cust_npwp').val(data['HEADER'][0]['tl_cust_npwp']);
						$('#tl_cust_address').val(data['HEADER'][0]['tl_cust_address']);
						

											
						// VESSEL
						$('#vessel').val(data['HEADER'][0]['tl_vessel_name']);
						$('#nama_agen').val(data['HEADER'][0]['tl_vessel_agent_name']);
						$('#no_pkk').val(data['HEADER'][0]['tl_vessel_pkk']);
						$('#voyage_in').val(data['HEADER'][0]['tl_voyin']);
						$('#voyage_out').val(data['HEADER'][0]['tl_voyout']);
						$('#port_of_loading').val(data['HEADER'][0]['tl_pol']);
						$('#port_of_destination').val(data['HEADER'][0]['tl_pod']);
						  ...
						  var i = 1;
							...
							$('#detail-list tbody').append(
								'<tr>' +
									...
									'<td>'+ abc.tl_dtl_del_via_name+'</td>' +
									'<td>'+ abc.tl_dtl_rec_date+'</td>' +
