...
<script>
	      	...
	      	//for (var i = 0, row; row = table.rows[i]; i++) {
	            ...
	            $("input:checkbox[name=check]:checked").each(function () {
	              // console.log('ad');
			 		...
			 		var temp = {
			 			"CANCL_DTL_ID":$('#stuff_dtl_id' + $(this).val() ).val(),
	...
	$.ajax({
      ...
      url: "<?=ROOT?>npksbilling/stuffingbatal/preview_edit/"+id_param,
      type: "GET",
      dataType : 'json',
      success: function (data) {
      	//console.log(data);
        ...
        console.log(data);
						...
						var req_to = '';
						if ((data['HEADER_REQ'][0]['stuff_from']== 1)){
							req_to = "DEPO";
						} else if ((data['HEADER_REQ'][0]['stuff_from']== 2)){
							req_to = "TPK"
						} else {
							req_to = "TONGKANG";
						}

       					$('#stuff_branch_id').val(data['HEADER_REQ'][0]['stuff_branch_id']);
						$('#stuff_branch_code').val(data['HEADER_REQ'][0]['stuff_branch_code']);
						...
						$('#pbm').val(data['HEADER_REQ'][0]['stuff_pbm_name']);
						
						$('#from').val(req_to);
						$('#penumpukan_oleh').val(data['HEADER_REQ'][0]['stuff_btl_from']);
						$('#payment_method').val(req_method);

						//customer
						$('#stuff_cust_name').val(data['HEADER_REQ'][0]['stuff_cust_name']);
						$('#stuff_cust_npwp').val(data['HEADER_REQ'][0]['stuff_cust_npwp']);
						$('#stuff_cust_address').val(data['HEADER_REQ'][0]['stuff_cust_address']);
						

											
						// VESSEL
						$('#vessel').val(data['HEADER_REQ'][0]['stuff_vessel_name']);
						$('#nama_agen').val(data['HEADER_REQ'][0]['stuff_vessel_agent_name']);
						$('#no_pkk').val(data['HEADER_REQ'][0]['stuff_vessel_pkk']);
						$('#voyage_in').val(data['HEADER_REQ'][0]['stuff_voyin']);
						$('#voyage_out').val(data['HEADER_REQ'][0]['stuff_voyout']);
						$('#port_of_loading').val(data['HEADER_REQ'][0]['stuff_vessel_pol']);
						$('#port_of_destination').val(data['HEADER_REQ'][0]['stuff_vessel_pod']);
		        	...
		        	//console.log(row);
						...
						// DETAILS
						  ...
						  var no = 1;
							...
							//if(abc.stuff_dtl_iscancelled === "Y" ) document.getElementById("chk").checked == true;
