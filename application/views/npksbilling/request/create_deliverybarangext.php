		...
		<div class="main-box">
			...
			<div class="main-box-body clearfix">
				<div class="form-group col-xs-6">
					...
					<input name="DEL_CARGO_EXT_FROM" id="DEL_CARGO_EXT_FROM" type="text" class="form-control" placeholder="Autocomplete" style="text-transform: uppercase;">
				</div>
				<div class="form-group col-xs-6">
...
<script>
	...
	$(document).ready(function() {
		...
		$('#DEL_CARGO_EXT_FROM').autocomplete({
			source: function( request, response ) {
				$.ajax({
					url: "<?=ROOT?>npksbilling/deliverybarangext/get_nomor_request",
					type: 'GET',
					dataType: "json",
					data: {
						search: request.term
			...
			select: function (event, ui) {
				$('#DEL_CARGO_EXT_FROM').val(ui.item.label);
				
				$.blockUI();

				$.ajax({
					url: "<?=ROOT?>npksbilling/deliverybarangext/auto_generate_by_noreq",
					type: 'GET',
					dataType: "json",
					data: {
					...
					success: function( data ) {
						...
						$('#DEL_CARGO_EXT_FROM_DATE').val(data['HEADER'][0]['del_cargo_date']);
						$('.auto-generate').removeClass('hidden_content');
						$("#DEL_CARGO_EXT_FROM").attr('disabled','disabled');
						$('#DEL_CARGO_DATE').val(today);
	...
	function save_extension() {
		...
		var details = [];
		var files = [];
		var del_cargo_no = $('#DEL_CARGO_EXT_FROM').val();
		...
		$('#detail-list tbody tr').each(function(){
			...
			var temp = {
				"DEL_CARGO_DTL_ID": null,
                "DEL_CARGO_HDR_ID": null,
