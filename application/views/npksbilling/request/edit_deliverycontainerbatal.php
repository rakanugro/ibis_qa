		...
		<div class="main-box">
			...
			<div class="main-box-body clearfix">
				...
				<div class="form-group col-xs-6">
					...
					<input name="del_branch_id" id="del_branch_id" type="hidden" class="form-control" placeholder="del_branch_id" required="" readonly="">
					<input name="del_branch_code" id="del_branch_code" type="hidden" class="form-control" placeholder="del_branch_code" required="" readonly="">
				</div>
			</div>
		</div>
	</div>
...
<script>
	      	...
	      	//for (var i = 0, row; row = table.rows[i]; i++) {
	            ...
	            $("input:checkbox[name=check]:checked").each(function () {
	              // console.log('ad');
			 		...
			 		var temp = {
			 			"CANCL_DTL_ID":$('#del_dtl_id' + $(this).val() ).val(),
	         		...
	         		arrDataEdit = {
                            ...
                            "update" : 
                            {
                                "DEL_DTL_ISCANCELLED" : "Y"
                            },
                            "where" : 
                            {
                                "DEL_DTL_ID" : $('#del_dtl_id' + $(this).val()).val()
                            }
                        };

        $.ajax({
			url: "<?=ROOT?>npksbilling/deliverycontainerbatal/update_request",
			type: 'POST',
			dataType: 'json',
			data: { 
	...
	//}
		...
		arrData = 
			{
				...
				"service_branch_id" : $('#del_branch_id').val(),
				"service_branch_code" : $('#del_branch_code').val() ,
				"data": [
					"HEADER",
					"DETAIL"
				...
				"HEADER": {
					...
					"VALUE": [{
						...
						"CANCELLED_STATUS": 1,
						"CANCELLED_TYPE" : 2,
						"CANCELLED_BRANCH_ID" : $('#del_branch_id').val() ,
						"CANCELLED_BRANCH_CODE" : $('#del_branch_code').val(), 
						"APP_ID" : 2 
					}]
				},
		...
		$.ajax({
			url: "<?=ROOT?>npksbilling/deliverycontainerbatal/save_request",
			type: 'POST',
			dataType: 'json',
			data: { 
				'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
			...
			success: function( data ) {
				...
				if (data.success === 'S') {
					...
					var cancelled_req_no = response.header.del_no;
					...
					setTimeout(function(){ window.location = "<?=ROOT?>npksbilling/deliverycontainerbatal/"; }, 2000);	
				} else {
					$.unblockUI();
					alert('Data Gagal Disimpan;');
				}
	...
	$.ajax({
      ...
      url: "<?=ROOT?>npksbilling/deliverycontainerbatal/preview_edit/"+id_param,
      type: "GET",
      dataType : 'json',
      success: function (data) {
      	//console.log(data);
        ...
        console.log(data);
						...
						var req_to = '';
						if ((data['HEADER'][0]['del_from'] == 1)){
							req_to = "DEPO";
						}else if((data['HEADER'][0]['del_from'] == 2)){
							req_to = "TPK";
						}else{
							req_to = "TONGKANG";
						}

        				$('#del_branch_id').val(data['HEADER_REQ'][0]['del_branch_id']);
						$('#del_branch_code').val(data['HEADER_REQ'][0]['del_branch_code']);
						$('#nomor_request').val(data["HEADER"][0]['cancelled_no']);

       					
       					$('#cancelled_id').val(data["HEADER"][0]['cancelled_id']);
       					$('#request_id_reff').val(data["HEADER_REQ"][0]['del_no']);
       					
						$('#pbm').val(data['HEADER_REQ'][0]['del_pbm_name']);
						
						$('#from').val(req_to);
						$('#penumpukan_oleh').val(data['HEADER_REQ'][0]['del_stackby_name']);
						$('#payment_method').val(req_method);

						//customer
						$('#del_cust_name').val(data['HEADER_REQ'][0]['del_cust_name']);
						$('#del_cust_npwp').val(data['HEADER_REQ'][0]['del_cust_npwp']);
						$('#del_cust_address').val(data['HEADER_REQ'][0]['del_cust_address']);
						

											
						// VESSEL
						$('#vessel').val(data['HEADER_REQ'][0]['del_vessel_name']);
						$('#nama_agen').val(data['HEADER_REQ'][0]['del_vessel_agent_name']);
						$('#no_pkk').val(data['HEADER_REQ'][0]['del_vessel_pkk']);
						$('#voyage_in').val(data['HEADER_REQ'][0]['del_voyin']);
						$('#voyage_out').val(data['HEADER_REQ'][0]['del_voyout']);
						$('#port_of_loading').val(data['HEADER_REQ'][0]['del_vessel_pol']);
						$('#port_of_destination').val(data['HEADER_REQ'][0]['del_vessel_pod']);
		        	...
		        	//console.log(row);
						...
						// DETAILS
						  ...
						  var no = 1;
							...
							//if(abc.del_dtl_iscancelled === "Y" ) document.getElementById("chk").checked == true;

							$('#detail-list tbody').append(
								'<tr>' +
									'<td>'+ no++ +'</td>' +
									'<td>'+ abc.del_dtl_cont +'</td>' +
									'<td>'+ abc.del_dtl_cont_size+'</td>' +
									'<td>'+ abc.del_dtl_cont_type+'</td>' +
									'<td>'+ abc.del_dtl_cont_status+'</td>' +
									'<td>'+ abc.del_dtl_cont_danger+'</td>' +
									'<td>'+ abc.del_dtl_cmdty_name +'</td>' +
									'<td>'+ abc.del_dtl_via+'</td>' +
									'<td>'+ abc.del_dtl_date_plan+'</td>' +
									'<td>'+ abc.del_dtl_owner_name +'</td>' +
									
									'<td>' +
										'<input type="checkbox" class="cb" name="check" id="chk'+i+'"  value="'+i+'" checked />' +
										'<input type="hidden" name="name1" id="del_dtl_id'+i+'" value="'+abc.del_dtl_id +'"/>' +
										'<input type="hidden" name="name2" id="dtl_cont'+i+'" value="'+abc.del_dtl_cont +'"/>' +
