		...
		<div class="main-box">
			...
			<div class="main-box-body clearfix">
				...
				<div class="form-group col-xs-6">
					...
					">
					<input name="tl_branch_id" id="tl_branch_id" type="hidden" class="form-control" placeholder="tl_branch_id" required="" readonly="
					">
					<input name="tl_branch_code" id="tl_branch_code" type="hidden" class="form-control" placeholder="tl_branch_code" required="" readonly="
					">
				</div>
			</div>
		</div>
		...
		<div class="main-box">
			...
			<div class="main-box-body clearfix">
				<div class="form-group col-xs-12">
					...
					<input name="tl_cust_name" id="tl_cust_name" type="text" class="form-control" placeholder="Autocomplete" required="" required="">
				</div>
				<div class="form-group col-xs-12">
					<label>NPWP</label>
					<input name="tl_cust_npwp" id="tl_cust_npwp" type="text" class="form-control" placeholder="Auto Generate" required="" readonly="">
				</div>
				<div class="form-group col-xs-12">
					<label>Alamat</label>
					<input name="tl_cust_address" id="tl_cust_address" type="text" class="form-control" placeholder="Autocomplete" required="" readonly="">
				</div>
...
<script>
	      	...
	      	//for (var i = 0, row; row = table.rows[i]; i++) {
	            ...
	            $("input:checkbox[name=check]:checked").each(function () {
	              // console.log('ad');
			 		...
			 		var temp = {
			 			"CANCL_DTL_ID":$('#tl_dtl_id' + $(this).val() ).val(),
	         		...
	         		arrDataEdit = {
                            ...
                            "update" : 
                            {
                                "TL_DTL_ISCANCELLED" : "Y"
                            },
                            "where" : 
                            {
                                "TL_DTL_ID" : $('#tl_dtl_id' + $(this).val()).val()
                            }
                        };

        $.ajax({
			url: "<?=ROOT?>npksbilling/trucklosingbatal/update_request",
			type: 'POST',
			dataType: 'json',
			data: { 
	...
	//}
		...
		arrData = 
			{
				...
				"service_branch_id" : $('#tl_branch_id').val(),
				"service_branch_code" : $('#tl_branch_code').val() ,
				"data": [
					"HEADER",
					"DETAIL"
				...
				"HEADER": {
					...
					"VALUE": [{
						...
						"CANCELLED_STATUS": 1,
						"CANCELLED_TYPE" : 9,
						"CANCELLED_BRANCH_ID" : $('#tl_branch_id').val() ,
						"CANCELLED_BRANCH_CODE" : $('#tl_branch_code').val(), 
						"APP_ID" : 2 
					}]
				},
		...
		$.ajax({
			url: "<?=ROOT?>npksbilling/trucklosingbatal/save_request",
			type: 'POST',
			dataType: 'json',
			data: { 
				'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
			...
			success: function( data ) {
				...
				if (data.success === 'S') {
					...
					var cancelled_req_no = response.header.tl_no;
					...
					setTimeout(function(){ window.location = "<?=ROOT?>npksbilling/trucklosingbatal/"; }, 2000);	
				} else {
					$.unblockUI();
					alert('Data Gagal Disimpan;');
				}
	...
	$.ajax({
      ...
      url: "<?=ROOT?>npksbilling/trucklosingbatal/preview_edit/"+id_param,
      type: "GET",
      dataType : 'json',
      success: function (data) {
      	//console.log(data);
        ...
        console.log(data);
        				...
        				var req_method = (data['HEADER'][0]['tl_cargo_paymethod'] != 2)? "PIUTANG" : "ADVANCED PAYMENT";
						var req_to = '';
						if ((data['HEADER'][0]['tl_cargo_from'] == 1)){
							req_to = "DEPO";
						}else if((data['HEADER'][0]['tl_cargo_from'] == 2)){
							req_to = "TPK";
						}else{
							req_to = "TONGKANG";
						}
       					
       					$('#tl_branch_id').val(data['HEADER_REQ'][0]['tl_branch_id']);
						$('#tl_branch_code').val(data['HEADER_REQ'][0]['tl_branch_code']);

       					$('#cancelled_id').val(data["HEADER"][0]['cancelled_id']);
       					$('#request_id_reff').val(data["HEADER_REQ"][0]['tl_no']);
       					$('#nomor_request').val(data["HEADER"][0]['cancelled_no']);
						$('#pbm').val(data['HEADER_REQ'][0]['tl_pbm_name']);
						
						$('#from').val(req_to);
						$('#penumpukan_oleh').val(data['HEADER_REQ'][0]['tl_stackby_name']);
						$('#payment_method').val(req_method);

						//customer
						$('#tl_cust_name').val(data['HEADER_REQ'][0]['tl_cust_name']);
						$('#tl_cust_npwp').val(data['HEADER_REQ'][0]['tl_cust_npwp']);
						$('#tl_cust_address').val(data['HEADER_REQ'][0]['tl_cust_address']);
						

											
						// VESSEL
						$('#vessel').val(data['HEADER_REQ'][0]['tl_vessel_name']);
						$('#nama_agen').val(data['HEADER_REQ'][0]['tl_vessel_agent_name']);
						$('#no_pkk').val(data['HEADER_REQ'][0]['tl_vessel_pkk']);
						$('#voyage_in').val(data['HEADER_REQ'][0]['tl_voyin']);
						$('#voyage_out').val(data['HEADER_REQ'][0]['tl_voyout']);
						$('#port_of_loading').val(data['HEADER_REQ'][0]['tl_vessel_pol']);
						$('#port_of_destination').val(data['HEADER_REQ'][0]['tl_vessel_pod']);
		        	...
		        	//console.log(row);
						...
						// DETAILS
						  ...
						  var no = 1;
							...
							//if(abc.tl_dtl_iscancelled === "Y" ) document.getElementById("chk").checked == true;

							$('#detail-list tbody').append(
								'<tr>' +
									'<td>'+ no++ +'</td>' +
									'<td>'+ abc.tl_dtl_cont +'</td>' +
									'<td>'+ abc.tl_dtl_cont_size+'</td>' +
									'<td>'+ abc.tl_dtl_cont_type+'</td>' +
									'<td>'+ abc.tl_dtl_cont_status+'</td>' +
									'<td>'+ abc.tl_dtl_cont_danger+'</td>' +
									'<td>'+ abc.tl_dtl_cmdty_name +'</td>' +
									'<td>'+ abc.tl_dtl_via+'</td>' +
									'<td>'+ abc.tl_dtl_date_plan+'</td>' +
									'<td>'+ abc.tl_dtl_owner_name +'</td>' +
									
									'<td>' +
										'<input type="checkbox" class="cb" name="check" id="chk'+i+'"  value="'+i+'" checked />' +
										'<input type="hidden" name="name1" id="tl_dtl_id'+i+'" value="'+abc.tl_dtl_id +'"/>' +
										'<input type="hidden" name="name2" id="dtl_cont'+i+'" value="'+abc.tl_dtl_cont +'"/>' +
