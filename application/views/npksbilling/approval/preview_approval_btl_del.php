    ...
    <div class="col-lg-12">
        <div class="main-box">
            <header class="main-box-header clearfix">
                <h2><span class="glyphicon glyphicon-folder-open"></span>&nbsp;&nbsp;<b>Delivery Container Reference</b></h2>
            </header>
...
<script>
    ...
    function save_approval(action, remarks) {
        ...
        var urlaction = "";
        if (action == 'approve') {
            urlaction = "<?php echo ROOT ?>npksbilling/appbtldel/approve/" + $('#CANCEL_ID').val();
        } else {
            urlaction = "<?php echo ROOT ?>npksbilling/appbtldel/reject/" + $('#CANCEL_ID').val();
        }
        if (action == 'approve') {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (array_data['Success'] == undefined) {
                                ...
                                approvebtl_del_log(no_req);
                                setTimeout(function() {
                                    window.location = "<?= ROOT ?>npksbilling/appbtldel";
                                }, 1000);
                            } else {
    ...
    function rejecbtl_del_log(no_req) {
        $.ajax({
            url: "<?= ROOT ?>npksbilling/transaction_log/rejecbtl_del_log",
            type: 'POST',
            data: {
                '<?php echo $this->security->get_csrf_token_name(); ?>': '<?php echo $this->security->get_csrf_hash(); ?>',
                no_req: no_req,
    ...
    function approvebtl_del_log(no_req) {
        $.ajax({
            url: "<?= ROOT ?>npksbilling/transaction_log/approvebtl_del_log",
            type: 'POST',
            data: {
                '<?php echo $this->security->get_csrf_token_name(); ?>': '<?php echo $this->security->get_csrf_hash(); ?>',
                no_req: no_req,
    ...
    $(document).ready(function() {
        ...
        //getdata
        $.ajax({
            url: "<?= ROOT ?>npksbilling/appbtldel/get_data/<?= $del_id ?>",
            type: 'GET',
            dataType: 'json',
            success: function(data) {
                $.unblockUI();
                if (data.HEADER != "") {
                    ...
                    arrData.HEADER.forEach(function(item, index) {
                        $("#DEL_ID_REFF").val(item.del_id);
                        $("#DEL_NO_REFF").val(item.del_no);
                        $("#DEL_CREATE_BY").val();
                        $("#DEL_DATE_REFF").val(item.del_date);
                    ...
                    arrData.DETAIL.forEach(function(abc) {
                        if (abc.del_dtl_cmdty_name == null) {
                            $label_cmdty_name = "N/A";
                        } else {
                            $label_cmdty_name = abc.del_dtl_cmdty_name;
                        }
        ...
        //getcancelid
        $.ajax({
            url: "<?= ROOT ?>npksbilling/appbtldel/get_cancel_id/<?= $del_no ?>",
            type: 'GET',
            dataType: 'json',
            success: function(data) {
                $.unblockUI();
