    ...
    <div class="col-lg-12">
        <div class="main-box">
            <header class="main-box-header clearfix">
                <h2><span class="glyphicon glyphicon-folder-open"></span>&nbsp;&nbsp;<b>Plugging Reefer reference</b></h2>
            </header>
    ...
    <div class="col-lg-12">
        <div class="main-box">
            ...
            <div class="main-box-body clearfix">
                ...
                <div class="form-group col-xs-6">
                    <label>ETA</label>
                    <input name="PLUG_ETA" id="PLUG_ETA" type="text" class="form-control" placeholder="Port Of Loading" required="" readonly="">
                </div>
                <div class="form-group col-xs-6">
                    <label>ETD</label>
                    <input name="PLUG_ETD" id="PLUG_ETD" type="text" class="form-control" placeholder="Port Of Destination" required="" readonly="">
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function save_approval(action, remarks) {
        ...
        var urlaction = "";
        if (action == 'approve') {
            urlaction = "<?php echo ROOT ?>npksbilling/appbtlplug/approve/<?php echo $cancel_id; ?>";
        } else {
            urlaction = "<?php echo ROOT ?>npksbilling/appbtlplug/reject/<?php echo $cancel_id; ?>";
        }
        if (action == 'approve') {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (array_data['Success'] == undefined) {
                                ...
                                approvebtl_plug_log(no_req);
                                setTimeout(function() {
                                    window.location = "<?= ROOT ?>npksbilling/appbtlplug";
                                }, 1000);
                            } else {
    ...
    function rejecbtl_plug_log(no_req) {
        $.ajax({
            url: "<?= ROOT ?>npksbilling/transaction_log/rejecbtl_plug_log",
            type: 'POST',
            data: {
                '<?php echo $this->security->get_csrf_token_name(); ?>': '<?php echo $this->security->get_csrf_hash(); ?>',
                no_req: no_req,
    ...
    function approvebtl_plug_log(no_req) {
        $.ajax({
            url: "<?= ROOT ?>npksbilling/transaction_log/approvebtl_plug_log",
            type: 'POST',
            data: {
                '<?php echo $this->security->get_csrf_token_name(); ?>': '<?php echo $this->security->get_csrf_hash(); ?>',
                no_req: no_req,
    ...
    $(document).ready(function() {
        ...
        if (id_plug != "") {
            $.ajax({
                url: "<?= ROOT ?>npksbilling/appbtlplug/update_plug/<?= $id ?>",
                type: 'GET',
                dataType: 'json',
                success: function(data) {
                    $.unblockUI();
                    if (data.HEADER != "") {
                        ...
                        arrData.HEADER.forEach(function(item, index) {
                            //$("#PLUG_ID").val(item.cancelled_req_no);
                            $("#cust_name").val(item.plug_cust_name);
                            $("#cust_npwp").val(item.plug_cust_npwp);
                            $("#cust_address").val(item.plug_cust_address);
                            $("#PLUG_NO").val(item.plug_no);
                            $("#tgl_request").val(item.plug_create_date);
                            $("#PLUG_STATUS").val(1);
                        ...
                        arrData.DETAIL.forEach(function(detail, index) {
                            ...
                            $('#detail-list tbody').append(
                                '<tr>' +
                                '<td>' + detail.plug_dtl_cont + '</td>' +
                                '<td>' + detail.plug_dtl_cont_size + '</td>' +
                                '<td>' + detail.plug_dtl_cont_type + '</td>' +
                                '<td>' + detail.plug_dtl_cont_status + '</td>' +
                                '<td>' + detail.plug_dtl_cont_danger + '</td>' +
                                '<td>' + detail.plug_dtl_cmdty_name + '</td>' +

                                '<td>' + detail.plug_dtl_activity_date + '</td>' +
