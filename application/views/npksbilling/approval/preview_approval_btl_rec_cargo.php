    ...
    <div class="col-lg-12">
        <div class="main-box">
            <header class="main-box-header clearfix">
                <h2><span class="glyphicon glyphicon-folder-open"></span>&nbsp;&nbsp;<b>Receiving Barang Reference</b></h2>
            </header>
...
<script>
    ...
    function save_approval(action, remarks) {
        ...
        var urlaction = "";
        if (action == 'approve') {
            urlaction = "<?php echo ROOT ?>npksbilling/appbtlreccargo/approve/" + $('#CANCEL_ID').val();
        } else {
            urlaction = "<?php echo ROOT ?>npksbilling/appbtlreccargo/reject/" + $('#CANCEL_ID').val();
        }
        if (action == 'approve') {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (array_data['Success'] == undefined) {
                                ...
                                approvebtlrec_cargo_log(no_req);
                                setTimeout(function() {
                                    window.location = "<?= ROOT ?>npksbilling/appbtlreccargo";
                                }, 1000);
                            } else {
    ...
    function rejecbtlrec_cargo_log(no_req) {
        $.ajax({
            url: "<?= ROOT ?>npksbilling/transaction_log/rejecbtlrec_cargo_log",
            type: 'POST',
            data: {
                '<?php echo $this->security->get_csrf_token_name(); ?>': '<?php echo $this->security->get_csrf_hash(); ?>',
                no_req: no_req,
    ...
    function approvebtlrec_cargo_log(no_req) {
        $.ajax({
            url: "<?= ROOT ?>npksbilling/transaction_log/approvebtlrec_cargo_log",
            type: 'POST',
            data: {
                '<?php echo $this->security->get_csrf_token_name(); ?>': '<?php echo $this->security->get_csrf_hash(); ?>',
                no_req: no_req,
    ...
    $(document).ready(function() {
        ...
        //getdata
        $.ajax({
            url: "<?= ROOT ?>npksbilling/appbtlreccargo/get_data/<?= $rec_id ?>",
            type: 'GET',
            dataType: 'json',
            success: function(data) {
                $.unblockUI();
                if (data.HEADER != "") {
                    ...
                    arrData.HEADER.forEach(function(item, index) {
                        $("#REC_ID_REFF").val(item.rec_cargo_id);
                        $("#REC_NO_REFF").val(item.rec_cargo_no);
                        $("#REC_CREATE_BY").val();
                        $("#REC_DATE_REFF").val(item.rec_cargo_date);
                    ...
                    arrData.DETAIL.forEach(function(abc) {
                        ...
                        $('#detail-list tbody').append(
                            ...
                            '<td>' + abc.rec_cargo_dtl_via_name + '</td>' +
                            '<td>' + abc.rec_cargo_dtl_stack_area_name + '</td>' +
        ...
        //getcancelid
        $.ajax({
            url: "<?= ROOT ?>npksbilling/appbtlreccargo/get_cancel_id/<?= $rec_no ?>",
            type: 'GET',
            dataType: 'json',
            success: function(data) {
                $.unblockUI();
