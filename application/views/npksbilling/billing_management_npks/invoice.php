	...
	<div class="col-lg-12">
		<div class="main-box clearfix">
			<header class="main-box-header clearfix">
				<h2 class="pull-left">Invoice List</h2>
			</header>
...
<script>
	...
	$(document).ready(function() {
		...
		$.ajax({
			type: "GET",
			url: "<?= ROOT ?>npksbilling/invoice/getListInvoice?",
			data: {},
			success: function(data) {
				...
				for (i = 0; i < jmlresponse; i++) {
					...
					var nota_paid_date = obj['result'][i]['nota_paid_date'];
				...
				obj['result'].forEach(function(abc) {
					table.append(
						...
						'<td>' + abc.nota_paid_date + '</td>' +
						'<td>' + '<b style="color:blue;">' + abc.reff_name + '</b>' + '</td>' +
						'<td style="font-size: 15.7px;">' +
						'<a class="btn btn-danger print-log" data-id="' + abc.nota_req_no + '" target="_blank" data-toggle="tooltip" title="Print Data" href="<?= apiUrl ?>/print/notaNPKS/' + abc.nota_id + '"><span class="glyphicon glyphicon-print"></span></a>' +
						'</td>' +
						'</tr>'
					);
	...
	function print_invoice_log(id) {
		$.ajax({
			url: "<?= ROOT ?>npksbilling/transaction_log/print_invoice_log",
			type: 'POST',
			//dataType: 'json',
			data: {
