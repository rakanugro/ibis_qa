...
<script>
	$(document).ready(function() {
		...
		$.ajax({
			type: "GET",
			url: "<?= ROOT ?>npksbilling/payment_cash/getListPaymentCash?",
			data: {},
			success: function(data) {
				...
				if (obj_nota_list['count'] > 0) {
					for (var a = 0; a < obj_nota_list['count']; a++) {
						...
						if (obj_nota_list['result'][a]['nota_status'] == "4" || obj_nota_list['result'][a]['nota_status'] == "1") {
							var isDisabled = "disabled";
						} else {
							var isDisabled = "";
						}

						if (obj_nota_list['result'][a]['nota_terminal'] == null) {
							var terminal = '';
						} else {
							var terminal = obj_nota_list['result'][a]['nota_terminal'];
						}

						if (obj_nota_list['result'][a]['nota_vessel_name'] == null) {
							var vess = '';
						} else {
							var vess = obj_nota_list['result'][a]['nota_vessel_name'];
						}

						if (obj_nota_list['result'][a]['nota_req_no'].substring(0, 3) != 'CNL') {
							table.append(
								...
								'<td>' +
								'<a class=\'btn btn-primary print-log\' data-id="' + obj_nota_list['result'][a]['nota_req_no'] + '" href="<?= ROOT ?>npksbilling/payment_cash/view_payment/' + obj_nota_list['result'][a]['nota_id'] + '-' + 'nota' + '/' + obj_nota_list['result'][a]['nota_paid'] + '" ' + isDisabled + '><span class="glyphicon glyphicon-list-alt" title="Confirmation"></span></a>' +
								'</td>' +
								'</tr>'
							);
						}
	...
	function cash_uper(id) {
		$.ajax({
			url: "<?= ROOT ?>npksbilling/transaction_log/cash_uper",
			type: 'POST',
			//dataType: 'json',
			data: {
